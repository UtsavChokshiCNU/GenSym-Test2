...
public class CutLowValueTest
{

	private CutLowValue testObj;

	@Before
	public void initObject()
	{
		testObj = new CutLowValue();
	}
	/**
	...
	{
		assertEquals("CutLowValue", testObj.getName());
	}
	...
	{
		final String description = "C = CutLowValue(A, B).  If A < B, then C = NaN, otherwise C = A.";
	...
	public void testEvaluate() throws MathException
	{
		...
		assertEquals(outputVals[0], Double.NaN, 0);
		assertEquals(outputVals[1], Double.MAX_VALUE, 0);
		assertEquals(outputVals[2], -1100, 0);
		assertEquals(outputVals[3], Double.NaN, 0);
	}
}
