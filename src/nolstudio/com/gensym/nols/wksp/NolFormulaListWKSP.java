...
public class NolFormulaListWKSP extends NolInternalFrame implements PreprocessorListener,InternalFrameListener {
...
/** Return the preprocessor of this WKSP.*/
...
/** initializeLayout. */
	private void initializeLayout() {
		...
		varDisplayChoice.addItemListener((ItemListener)actionListener);
		...
		locationDisplayChoice = new JComboBox ();
 		locationDisplayChoice.addItemListener((ItemListener)actionListener);
		locationPane.add(locationDisplayChoice);
		locationDisplayChoice.addItem("Anywhere");
		locationDisplayChoice.addItem("Outputs");
		locationDisplayChoice.addItem("Inputs");
		...
		formulaList.addListSelectionListener((ListSelectionListener)actionListener);
		formulaList.addMouseListener((MouseListener)actionListener);
	...
	private JPanel getControlPanel(){
		...
		but = new JButton("Export...");
		but.addActionListener(actionListener);
  		pane0.add(but);

		but = new JButton("Import...");
		but.addActionListener(actionListener);
  		pane0.add(but);
...
/*				if(formulaList.getMinSelectionIndex()!=formulaList.getMaxSelectionIndex()) {
		...
		public void itemStateChanged(ItemEvent e) {
			...
			if (e.getSource().equals(dataSeriesDisplayChoice)) {
				noFire = true;
				...
				formulaList.validate();
				noFire = false;
			}else if (e.getSource().equals(varDisplayChoice)) {
				noFire = true;
	...
	private void applyFormulaAction() {
		...
		Toolkit.getDefaultToolkit().beep();
//		frame.getProjectTree().setroject(); //it should not create any new object in the tree. Gchen 6/21/2002
	}
 ...
 /**  */
    ...
    class FormulaConstructor extends JDialog {
		protected NolTextField outputField;
		protected volatile NolTextField inputField;
...
/***/	private boolean inputFieldHasFocus;
		...
		private void initLayout(){
			...
			JLabel label1 = new JLabel("Outputs:");
			label1.setBounds(X0, Y0, W0, H0);
			workPane.add(label1);
			fieldFont = new Font("dialog", 0, 12);
			JLabel label2 = new JLabel("Formula:");
			label2.setBounds(X0, Y1, W0, H0);
			workPane.add(label2);
			newVarBut = new JButton("New...");
			...
			pane.setBorder(new TitledBorder(new EmptyBorder(1, 1, 1, 1),               
				"Functions:", TitledBorder.LEFT , TitledBorder.TOP));
			...
			pane.setBorder(BorderFactory.createTitledBorder("Numerical Inputs"));
			...
			addArgButt = new JButton("Add Arg"); 
			addArgButt.addActionListener(constActionListener);
			pane.add(addArgButt);

			deleteArgButt = new JButton("Del Arg"); 
			...
			helpText.setBackground(getContentPane().getBackground());
		...
		private NolTextField getOutputField() {
			if (outputField == null) {
				outputField = new NolTextField();
		...
		private NolTextField getInputField() {
			if (inputField == null) {
				inputField = new NolTextField();
		...
		protected void initializeEditContext() {
			...
			else editedInputIndex.removeAllElements();
...
/** The display the formula choice.*/
		private void displayVarListChoice(){
			...
			varListChoice.addElements(getPreprocessor().getVariableNames());
			varListChoice.addElements(getPreprocessor().getDerivedVariableNames());
		}

		
		// UNDO/REDO STUFF
		private void setText(NolTextField field, String newText) {
		...
		class EditState extends Object {
...
/** The adapter class for capture the formula list drag and drop actions.*/	
	...
	public class NolImportFormulaDialog  extends JDialog {
		...
		public NolImportFormulaDialog (NolMainFrame frame, Preprocessor p){
			...
			importFormulaPanel = new NolSelectPanel(" Formulas:");
		...
		private JPanel getSourcePanel(){
			...
			JRadioButton radio = new JRadioButton("From Preprocessor:",true);
			...
			radio = new JRadioButton("From File:");
			radio.addActionListener(importActionListener);
			group.add(radio);
			pane2.add(radio);

			fileImportBut = new JButton("File...");
