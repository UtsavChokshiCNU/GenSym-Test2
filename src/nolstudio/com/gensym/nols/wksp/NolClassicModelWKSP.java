...
public class NolClassicModelWKSP  extends NolInternalFrame 
    ...
    protected static final String MODELWKSP_REPEATRAINING = "ModelWKSP_RepeatTraining";
    ...
    protected static final String MODELWKSP_EXPORTSANN = "ModelWKSP_ExportWholeANN";
...
//    protected static final String MODELWKSP_STRUCTURE = "ModelWKSP_";
	...
	public NolClassicModelWKSP (NolMainFrame frame, NolModel2 model){
...
//		modelStructionDesciption.setBackground(Color.white);
	...
	private JPanel getControlPanel(){
		...
		if(model instanceof NolRbfnModel){
			but.setText(frame.getNolResourceString(MODELWKSP_REPEATRAINING));
		}else{
			but.setText(frame.getNolResourceString(MODELWKSP_CONTINUETRAINING));
		}
		...
		weightBut.addActionListener(actionListener);
  		pane0.add(weightBut);
		if(model instanceof NolAnnModel && ((NolAnnModel)model).getRunMode() == 1){
			but = new JButton(frame.getNolResourceString(MODELWKSP_EXPORTSANN));
			but.setActionCommand("Export ANN...");
			but.addActionListener(actionListener);
			pane0.setLayout(new GridLayout(7,1,5,5));
  			pane0.add(but);
		}
		...
		if(model instanceof NolRbfnModel && ((NolRbfnModel)model).isNormalizeData()){
	...
	private void initDataDisplay(){
		...
		if(model instanceof NolAnnModel){
			typeName.setText("Autoassociated Net");
		}else if(model instanceof NolBpnModel){
			typeName.setText("Backpropagation Net");
		}else if(model instanceof NolRbfnModel){
			typeName.setText("Radial Basis Net");
		}
		...
		if(model.isTrained()){
			if(model instanceof NolBpnModel || model instanceof NolRbfnModel){
	...
	private void getModelStatistics(NolModel2 m, Vector variables, Vector rmse, Vector corrcoef) {
		int digits = 4;
		...
		if(m instanceof NolBpnModel){
			VP = ((NolBpnModel)m).trainingResults;
		}else if(m instanceof NolRbfnModel){
    ...
    class ControlAction extends AbstractAction implements DocumentListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(command.equals("Continue Training...")){
				if(model instanceof NolBpnModel){
					frame.getBpnTrainDialog().setBpnModel((NolBpnModel)model);
					frame.getBpnTrainDialog().setVisible(true);	
				}else if(model instanceof NolRbfnModel){
					frame.getRbfnTrainDialog().setRbfnModel((NolRbfnModel)model);
					frame.getRbfnTrainDialog().setVisible(true);	
				}
			...
			}else if(command.equals("Export ANN...")){
				if(!model.isTrained()){
				...
				fileChooser.setDialogTitle("Export Whole ANN network");
				NolFileFilter filter1 = new NolFileFilter(
				new String[] {"txt"}, "ASCII ANN File"
				);
				...
				fileChooser.setSelectedFile(new File(model.getName()+".txt"));
				...
				try {
					if(model instanceof NolAnnModel){
						((NolAnnModel)model).writeANNToFile(path);
					}
				}catch (Exception evt) {
					frame.showOKDialog("Error in writing the ANN.");
				}
				...
				NolFileFilter filter1 = new NolFileFilter(
				new String[] {"txt"}, "ASCII Factor File"
				);
				fileChooser.addChoosableFileFilter(filter1);
				fileChooser.setFileFilter(filter1);
				fileChooser.setDialogType(JFileChooser.SAVE_DIALOG);
				...
				NolFileFilter filter1 = new NolFileFilter(
				new String[] {exten}, "ASCII Model Weight File"
				);
				...
				fileChooser.setSelectedFile(new File(model.getName()+"."+exten));
				fileChooser.setLocation(200,200);			
				int retval = fileChooser.showSaveDialog(frame);
				...
				// try to start writing
				try {
					if(model instanceof NolBpnModel || model instanceof NolAnnModel){
						((NolBpnModel)model).writeBpnToFile(path);
					}else if(model instanceof NolRbfnModel){
				...
				if(nm.length() > 0){
					...
					setTitle(title);
					if(model instanceof NolAnnModel){
						frame.getProjectTree().changeAnnModelName((NolAnnModel)model,true);
					}else if(model instanceof NolBpnModel){
						frame.getProjectTree().changeBpnModelName((NolBpnModel)model,true);
					}else if(model instanceof NolRbfnModel){
						frame.getProjectTree().changeRbfnModelName((NolRbfnModel)model,true);
					}
				}else{
					nm = model.getName();
