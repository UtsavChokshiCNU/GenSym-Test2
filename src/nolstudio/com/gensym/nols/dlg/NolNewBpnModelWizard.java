...
public class NolNewBpnModelWizard  extends NolWizardDialog
	...
	private static final String GENERAL_LABEL = "NewBpnModelWizard_GeneralLabel";
	...
	public NolNewBpnModelWizard (NolMainFrame dw) {
		super((JFrame)dw, "Create New Backpropagation Model", true, 7);
	...
	private JPanel getGeneralPanel(){
		...
		URL url = this.getClass().getResource("largebpn.gif");
...
//		autoPane.setBorder(BorderFactory.createTitledBorder("Automatically Select Variables & Calculate Delays:"));
	...
	public void setVisible(boolean v){
		...
		currentStep = 0;
		modifyStep(true);
		String title = "Create New Backpropagation Net - "+setTitleForStep(currentStep);
		title = title + " - General";
		setTitle(title);
		super.setVisible(v);
	}
...
//		private String[] colHd = {"Variable","Rating", "Used"};
	...
	private boolean resetStructTableModelParameter(){
		...
		structTableModel.setParameter(nout,inputCols);
		return true;
	}
	...
	protected boolean finishAction(){
		...
		if(nodes[0] != inputCols || nodes[nodes.length-1] != nout){
			frame.showOKDialog(frame.getNolResourceString(WRONGSTRUCTURE));
			return false;
		}

		ClassicBpn net = new ClassicBpn(nlayers,tf,nodes);
		model = new NolBpnModel(net,modelName.getText(),outs ,
