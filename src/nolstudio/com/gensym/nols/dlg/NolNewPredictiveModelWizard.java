...
//	private JButton advancedStruct;
	...
	private JCheckBox recursiveChoice;
	private JPanel dataseriesSourcePane;
	...
	private static final String GENERAL_LABEL = "NewPredictiveModelWizard_GeneralLabel";
	private static final String COMMENT_LABEL = "CommentLabel";
	private static final String NAMEEDITLABEL = "NewPredictiveModelWizard_NameEditLabel";
	private static final String RECURSIVECHOISE = "NewPredictiveModelWizard_RecursiveChoiceLabel";
	private static final String PREPROCESSOR_LABEL = "NewPredictiveModelWizard_PreprocessorLabel";
	private static final String PREPROCESSOR = "NewPredictiveModelWizard_Preprocessor";
	private static final String DATASOURCE_LABEL = "NewPredictiveModelWizard_OutputDataSourceLabel";
	private static final String DATASERIES = "NewPredictiveModelWizard_DataSeries";
	private static final String VARIABLE = "NewPredictiveModelWizard_Variable";
	private static final String SAMPLE = "NewPredictiveModelWizard_Sample";
	private static final String CLASSIFICATION_LABEL = "NewPredictiveModelWizard_ClassificationLabel";
	private static final String NODELAY_LABEL = "NewPredictiveModelWizard_NoDelayLabel";
	private static final String DELAY_LABEL = "NewPredictiveModelWizard_DelayLabel";
	private static final String HOUR = "NewPredictiveModelWizard_Hour";
	private static final String MINUTE = "NewPredictiveModelWizard_Minute";
	private static final String SECOND = "NewPredictiveModelWizard_Second";
	private static final String DELAYS = "NewPredictiveModelWizard_Delays";
	private static final String SETTING_LABEL = "NewPredictiveModelWizard_SettingLabel";
	private static final String SETTING_BORDER = "NewPredictiveModelWizard_SettingBorder";
	private static final String THRESHOLD = "NewPredictiveModelWizard_Threshold";
	private static final String CALCULATERATINGS = "NewPredictiveModelWizard_CalculateRatings";
	private static final String STRUCTURE_LABEL = "NewPredictiveModelWizard_StructureLabel";
	private static final String OUTPUT = "NewPredictiveModelWizard_Output";
	private static final String INPUT = "NewPredictiveModelWizard_Input";
	private static final String UNUSED = "NewPredictiveModelWizard_Unused";
	private static final String MINDELAY = "NewPredictiveModelWizard_MinDelay";
	private static final String MAXDELAY = "NewPredictiveModelWizard_MaxDelay";
	private static final String INTERVAL = "NewPredictiveModelWizard_Interval";
	private static final String DELAY = "NewPredictiveModelWizard_Delay";
	private static final String RATING = "NewPredictiveModelWizard_Rating";
	private static final String USED = "NewPredictiveModelWizard_Used";
	private static final String NOOUTPUTISSPECIFIED = "NewPredictiveModelWizard_NoOutputIsSpecified";
	private static final String NOINPUTISSPECIFIED = "NewPredictiveModelWizard_NoInputIsSpecified";
	private static final String NOLFINITEDATA = "NewPredictiveModelWizard_NoFiniteData";
	private static final String THRESHOLDTOOLTIP = "NewPredictiveModelWizard_ThresholdToolTip";
	private static final String LOADFROMEXISTMODEL = "NewPredictiveModelWizard_LoadFromModel";
	private static final String LOADFROMDISK = "NewPredictiveModelWizard_LoadFromDisk";
	private static final String PREDICTIVEMODEL = "NewPredictiveModelWizard_PredictiveModel";
	private static final String NOMODELSELECTED = "NewPredictiveModelWizard_NoModelSelected";
	private static final String NONAMEFORTHISMODEL = "NewPredictiveModelWizard_NoNameForThisModel";
...
//		super((JFrame)dw, "Create New Predictive Model", true, 7); //if DEBUG.more = true
		super((JFrame)dw, "Create New Predictive Model", true, 6);
...
//		stepPanes.setElementAt(getStructurePane(),6);
	...
	private JPanel getGeneralPanel(){
		...
		URL url = this.getClass().getResource("largePredictiveModel.gif");
...
//			;
		...
		JPanel titlePane = new JPanel(new GridLayout(2,1,10,5));
 		titlePane.setBorder(new EmptyBorder(10,10,10,10));
		
		JPanel namePane = new JPanel(new GridLayout(1,2,10,5));
...
//		namePane.add(new JLabel("Enter the name for this model:"));
		...
		titlePane.add(namePane);
		 
		titlePane.add(new JLabel(frame.getNolResourceString(COMMENT_LABEL)+":"));
 		cenPane.add("North",titlePane);
...
//		String s = "Select one data series that contains outputs for this model:";
		...
		dataseriesSourcePane = new JPanel(new GridLayout(1,2,10,10));
 		dataseriesSourcePane.setBorder(new EmptyBorder(0,0,10,0));
		dataseriesSourcePane.add(new JLabel(frame.getNolResourceString(DATASERIES)+":"));
		dataSeriesName = new JTextField();
		dataSeriesName.setEditable(false);
		dataseriesSourcePane.add(dataSeriesName);

		recursiveChoice = new JCheckBox();
		recursiveChoice.setHorizontalTextPosition(SwingConstants.LEFT);
		recursiveChoice.addActionListener(actionListener);

		cenPane.add("North",dataseriesSourcePane);
...
//		delPane.setBorder(BorderFactory.createTitledBorder("Delays:"));
		...
		delayTableView.setDefaultRenderer(String.class, 
			new DelayTableRenderer());
...
//				   "you are expert user.";
		...
		autoStructFlag.setActionCommand("Auto");
		group1.add(autoStructFlag);
  		autoStructFlag.addActionListener(actionListener);

 		manualStructFlag = new JRadioButton("PLS only");
		manualStructFlag.setActionCommand("PLS only");
		...
		structurePane.add("Center",selectPane);
...
//				   "you are expert user.";
  ...
  */
	...
	public void resetData(){
		...
		recursiveChoice.setSelected(false);
	...
	private void selectedPreprocessor(int selectedIndex){
		...
		if(hasTimeStamps){
//System.out.println("selectedPreprocessor setNames");
		...
		setRecursiveButtonChoice(hasTimeStamps);
	}

	private void setRecursiveButtonChoice(boolean hasTS){
		dataseriesSourcePane.removeAll();
		if(hasTS){
			dataseriesSourcePane.setLayout(new GridLayout(2,2,10,10));
			dataseriesSourcePane.add(new JLabel(frame.getNolResourceString(DATASERIES)+":"));
			dataseriesSourcePane.add(dataSeriesName);

			dataseriesSourcePane.add(new JLabel(frame.getNolResourceString(RECURSIVECHOISE)+"?"));
			dataseriesSourcePane.add(recursiveChoice);
		}else{
			dataseriesSourcePane.setLayout(new GridLayout(1,2,10,10));
			dataseriesSourcePane.add(new JLabel(frame.getNolResourceString(DATASERIES)+":"));
			dataseriesSourcePane.add(dataSeriesName);
		}
	...
	private void loadParaFromExistModel(){
		...
		models.addAll(frame.getProject().getPredictiveModels());
		models.addAll(frame.getProject().getBpnModels());
		models.addAll(frame.getProject().getRbfnModels());
		...
		if(getLoadFromExistModelDialog().isOk()){
			...
			if(index >=0){
				NolModel2 existModel = (NolModel2)models.elementAt(index);
...
//		System.out.println("before delay arrangement");
		...
		if(hasTimeStamps){
//System.out.println("updateParameterFromExistingParameter setNames");
			autoSelectionTableModel.setNames(usedVarNames);
			selectionSorter.setModel(autoSelectionTableModel);
		}else{
			autoSelectionNoDelayTableModel.setNames(usedVarNames);
...
//		System.out.println("before delay arrangement");
		...
		calculateDelay.setEnabled(true);
		if(hasTimeStamps){
//System.out.println("updateParameterFromExistingModel setNames");
			autoSelectionTableModel.setNames(usedVarNames);
			selectionSorter.setModel(autoSelectionTableModel);
		}else{
			autoSelectionNoDelayTableModel.setNames(usedVarNames);
	...
	class ControlAction extends AbstractAction implements ListSelectionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			}else if(e.getSource().equals(recursiveChoice)){
				refreshChecks(0,0);
//System.out.println("getSource = recursiveChoice");
			}
		}
	...
	public void receiveRatingResults(Vector inVars, Vector delays, Vector outVars, Vector ratings) {
		...
		if(hasTimeStamps){
//System.out.println("receiveRatingResults setNames");
			autoSelectionTableModel.setNames(usedVarNames);
			selectionSorter.setModel(autoSelectionTableModel);
		}else{
			autoSelectionNoDelayTableModel.setNames(usedVarNames);
	...
	private void refreshChecks(int row, int col){
		...
		if(hasTimeStamps){
			...
			if(recursiveChoice.isSelected() && !outVarNames.isEmpty()){
				for(int i=outVarNames.size()-1;i>=0;i--){
					usedVarNames.insertElementAt(outVarNames.elementAt(i),0);
				}
			}
			delayTableModel.resetNames(usedVarNames);
//System.out.println("refreshChecks setNames");
			autoSelectionTableModel.setNames(usedVarNames);
			selectionSorter.setModel(autoSelectionTableModel);
		}else{
			for(int i=0;i<vc;i++){
	...
	private void convertDelayFromTimeUnits(int from, int to){
		...
		if(hasTimeStamps){
			...
			autoSelectionTableModel.resetRating();
		}
		calculateDelay.setEnabled(true);
	}
	...
	class DelayTableRenderer extends JLabel 
		implements TableCellRenderer, java.io.Serializable{
		@Override
		public Component getTableCellRendererComponent(JTable table,
                                               Object value,
			...
			String name = (String)usedVarNames.elementAt(row);
			setOpaque(true);
...
//		private String[] colHd = {"Variable","Min Delay", "Max Delay","Interval"};
		...
		public void setValueAt(Object aValue, int r, int c) {
			...
			calculateDelay.setEnabled(true);
			if(hasTimeStamps){
//System.out.println("setValueAt setNames");
				autoSelectionTableModel.setNames(usedVarNames);
				selectionSorter.setModel(autoSelectionTableModel);
			}else{
				autoSelectionNoDelayTableModel.setNames(usedVarNames);
...
//			frame.showOKDialog("No input is specified.");
		...
		for(int i=0;i<c;i++){
			if(((JRadioButton)outchecks.elementAt(i)).isSelected()){
				...
				nout++;
				for(int j=0;j<dls.length;j++){
//System.out.println("varNames.elementAt(i)="+varNames.elementAt(i)+"   dls[j]="+dls[j]);
					if(dls[j]>0)
						inputCols++;
				}
...
//			frame.showOKDialog("No output is specified.");
		...
		if(ins.isEmpty()) {
			long delayInterval = delayIntervals[0];
			for (int i=0;i<delayIntervals.length;i++){
				if(delayInterval != delayIntervals[i]){
					frame.showOKDialog(frame.getNolResourceString("NewPredictiveModelWizard_ShouldHaveSameDelayInterval"));
					return false;
				}
			}
			model = new NolAutorecursiveModel(ensemble,modelName.getText(),outs ,
				ins,outDelays,inDelays, preprocessor);
			((NolAutorecursiveModel)model).setDelayInterval(delayInterval);
		}else{
			model = new NolModel2(ensemble,modelName.getText(),outs ,
				ins,outDelays,inDelays, preprocessor);
		}
...
System.out.println("newdls="+newdls.length);
for	(int k=0;k<newdls.length;k++){
System.out.println("newdls("+k+")="+newdls[k]);
}
System.out.println("dls="+dls.length);
for	(int k=0;k<dls.length;k++){
System.out.println("dls("+k+")="+dls[k]);
}
*/
			if(rr == rold){
