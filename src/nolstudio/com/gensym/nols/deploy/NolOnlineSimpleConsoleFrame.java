...
public class NolOnlineSimpleConsoleFrame extends JFrame implements EnsembleModelListener,
	ActionListener {
	...
	public NolOnlineSimpleConsoleFrame() {
		super();
...
//System.out.println("inside refreshPredActDisplay_1 1 ");
		if(Y.getRows() == 0 || YPred.getRows() == 0){
			return;
		}
//System.out.println("inside refreshPredActDisplay_1 2 ");
		if(Y.getRows() != YPred.getRows()){
			return;
		}
//System.out.println("inside refreshPredActDisplay_1 3");
		try{
			if(Y.getRows() > 1000){
...
//System.out.println("inside refreshPredActDisplay_1 4");
	...
	public void setModel1TrainOutput(Matrix M){
		if(Y != null)
			Matlib.clear(Y);
		Y = M;
		int numOut_1 = Y.getCols();
		varChoiceY_1.removeAllItems();
		for(int j=0; j < numOut_1; j++){
			varChoiceY_1.addItem("Output_"+(j+1));			
		}
		noFire = true;
		varChoiceY_1.setSelectedIndex(0);
		noFire = false;
...
//	System.out.println("start setChartOneData");

			setChartOneData(essembleTmes_1);
//	System.out.println("finish setChartOneData");
			refreshPredActDisplay_1();
//	System.out.println("finish refreshPredActDisplay_1");
			String s = "Ensemble model mean squared error: "+mpe +"\n";
//	System.out.println("appendTextLine s= " + s);
			appendTextLine(s, true); // add string s to console
		} catch (Exception me) {
			System.out.println("Non-Fatal exception: "+ me.toString());
...
//			me.printStackTrace();
	...
	public void trainingComplete(EnsembleModelEvent e){
		...
		NolConfirmDialog confirmDialog = new NolConfirmDialog(this,"Information",buttonLabels,"Finished training !", "info.gif");
