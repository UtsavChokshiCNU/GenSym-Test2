...
public class NolConsoleFrame extends JFrame implements EnsembleModelListener,
	ActionListener {
	...
	private JComboBox varChoiceY_1; // bbush 8-13-98
	private JComboBox varChoiceY_2; // bbush 8-13-98

	private JTextField trainingTimeField;
	private JCheckBox autoStopCheckBox;
	private JCheckBox singlePLSCheckBox;
...
//	private static ImageIcon pauseIcon; 
	...
	/** The Console Frame */
	public NolConsoleFrame(Project p, NolMainFrame frame, String modelName) {
		super();
		this.frame = frame;
		project = p;
		setTitle(String.format("NOL Console [%s]", modelName));
...
//		stopIcon = new ImageIcon(this.getClass().getResource("stop.gif"));
//		pauseIcon = new ImageIcon(this.getClass().getResource("pause.gif"));
//		ImageIcon consoleLogo = new ImageIcon(this.getClass().getResource("console.gif"));
	...
	private JPanel getChartPanelOne() {
		...
		JPanel inputsPanel = new JPanel(new FlowLayout(FlowLayout.LEADING, 10, 10));
		inputsPanel.setBorder(new EmptyBorder(0,25,0,0));
		inputsPanel.add(new JLabel(frame.getNolResourceString(NolTrainDialog.MAXTIME), SwingConstants.RIGHT));
		trainingTimeField = new JTextField(5);
		trainingTimeField.setEditable(false);
		inputsPanel.add(trainingTimeField);
		inputsPanel.add(new JLabel(frame.getNolResourceString(NolTrainDialog.AUTOMATICSTOP), SwingConstants.RIGHT));
		autoStopCheckBox = new JCheckBox();
		autoStopCheckBox.setEnabled(false);
		inputsPanel.add(autoStopCheckBox);
		inputsPanel.add(new JLabel(frame.getNolResourceString(NolTrainDialog.SINGLEPLSMODEL), SwingConstants.RIGHT));
		singlePLSCheckBox = new JCheckBox();
		singlePLSCheckBox.setEnabled(false);
		inputsPanel.add(singlePLSCheckBox);
		
		pane0.add("Center",inputsPanel);
		pane0.add("South",getConsolePanel_1());
...
/*	public void resetChart() {
		for(int i = 0; i < essembleTmes.length; i++) {
			essembleTmes[i] = Double.NaN;
		}
		setEnsembleModelErrorData(essembleTmes);
		
//		Y.set(Y.getRows(), Y.getCols(), Double.NaN);
//		YPred.set(YPred.getRows(), YPred.getCols(), Double.NaN);
//		refreshPredActDisplay();
		
		numModelsTrained_1 = 0;
		modelCountField_1.setText(String.valueOf(numModelsTrained));
		lastModelAdded_1 = 0;
		lastModelCountField_1.setText(String.valueOf(lastModelAdded));
	}
*/
...
/** Set the project of this project frame and refresh the display.*/
	...
	public void setModel(NolModel2 m) {
		theModel = m;
...
//	public synchronized void modelAdded(EnsembleModelEvent e) {
		...
		if(ensemble.equals(theModel.getModel())) { // the event came from the Output Model
			...
			String s = "Ensemble model chi-square error: "+mpe +"\n";
	...
	public void trainingComplete(EnsembleModelEvent e){
		...
		if(withTwoModels){ 
			...
			stopButton.setEnabled(false);
			frame.finishTrainingModel(theModel);		
			frame.showOKDialog("Finished training "+theModel.getName()+" !");		
		}
	}
	...
	public void setTrainingTime(long trainingTime) {
		trainingTimeField.setText(String.valueOf(trainingTime/60000l));
	}

	public void setUseAutomaticStopping(boolean useAutomaticStopping) {
		autoStopCheckBox.setSelected(useAutomaticStopping);
	}

	public void setSinglePLSModel(boolean singlePLSModel) {
		singlePLSCheckBox.setSelected(singlePLSModel);
	}
