 ...
 * G2-to-G2 benchmarks of CPM and JRA, and contains a set of RPC
 * functions taking or not taking arguments, returning no or some
 * values, and calling into or out of G2.
 */

/*
 * jh, 6/10/92.
 * Attempt to get the GSI 2.0 benchmarks working in 3.0.  Changes are
 * noted where they appear (look for "jh, 6/").  In addition, had to
 * make everything in this and gsi_regress_for_benchmark return
 * GSI_SUCCESS.  On the KB side (in
 * ma:>jh>gsi>regression>gsi-benchmarks.kb), removed the "allow other
 * processing" when multiple RPCs were launched for the RPC throughput
 * tests, since they returned so fast that we never ramped up to the
 * "clogged" state we needed to start the measurement.
 */
...
int                index;
{
 ...
 time_arg.error_cd = 0; /* Added.  jh, 6/10/92. */
...
int                index;
{
 TIME_ARG_TYPE      time_arg;
 /*
  * jh, 6/10/92.  The following used to declare VECTOR_TYPE in the 2.0
  * benchmarks.  I can't see how that would have worked, but I do know
  * that the GSI struct declarations underwent a change since then.
  */
 T_VECTOR_TYPE      attribute_description[NUMBER_OF_VECTOR_ATTRIBUTES];
 int                j;

 time_arg.obj_index = index;
 time_arg.error_cd = 0; /* Added after 2.0 benchmark.  jh, 6/10/92. */
...
/* 
 * jh, 6/10/92.  The following timestamping statements were not in the
 * 2.0 benchmark.  I believe can include in the 3.0 benchmark, since
 * they were probably lost in the noise then.  Indeed, they appear to
 * be necessary for proper operation now.
 */
   attribute_description[j].is_timed = 1;
   attribute_description[j].date.year = 1990;
   attribute_description[j].date.month = 12;
   attribute_description[j].date.day = 17;
   attribute_description[j].date.hour = 12;
   attribute_description[j].date.minute = 0;
   attribute_description[j].date.second = 0;
  }
...
/* printf("returned values from get\n"); */
   return(GSI_SUCCESS);
  }
 ...
 if (!strcmp(first_sensor->param_1.p_value.symbol_p, "SET-DATA"))
  {
   ...
   gsi_ret_vals(arglist, argcount); 
/* printf("returned values from set\n"); */
   return(GSI_SUCCESS);
  }
...
/* jh, 6/5/92.  Added post-2.0 for 3.0 tests. */

main(argc, argv)
int     argc;
char   *argv[];
{
 SetIcpMode(ICP_USE_RPCS);
 gsimain(argc, argv);
}
