 ...
 *
 * $Header: /media/gensym/home/versata1/cvs_repo_copyDec06/ab-repos/ext/c/stdef.h,v 1.3 1996-01-25 05:43:56 vkp Exp $
 *
 * $Log: not supported by cvs2svn $
 * Revision 1.2  1995/11/27 21:57:07  mes
 * Project: libfnt Module Relocation
 * Witness: jv
 *
 * Removed Bitstream files, which are now in the CVS fnt module.
 *
 * Revision 1.1  1995/10/03 00:37:36  jra
 * Adding ext files from 4.0 Rev. 1 as initial commit for AB.
 *
 * Revision 1.3  1994/04/25 21:12:24  mpc
 * Project: Windows 3.1 Port
 * Modified Files: fontrip.c fontscal.h load_pcl.c out_scrn.c stdef.h tr_mkchr.c
 *
 * Discovered that in the Bitstream Font Library, instances of #ifdef MSDOS were
 * for code for the 16bit Microsoft compiler. Since we use WatcomC, none of this
 * is necessary. Have turned off the MSDOS flag in the makefile. Since Watcom does
 * param checking, some param mismatches needed to be resolved.
 *
 * Revision 1.2  1993/07/01  15:41:52  mpc
 * Project: Windows NT Port
 * Modified Files:  cprim.c cprim.h cstreams.c ctime.c g2signal.c nsleep.c
 *                  process.c stdef.h tcp.c
 *
 * Added #ifdef sections for Win32 functions.
 * cprim.c: Win32 implementation of hostname, username, homedir, and common
 * 	functions.
 * cprim.h: Win32 def of correct MEMZERO function.
 * cstreams.c: Added defs for Win32 file io functions.
 * ctime.c: Added defs for Win32 time function support.
 * g2signal.c: Added setup for Win32 signal handler.
 * nsleep.c: Implemented Win32 nsleep() call.
 * process.c: Win32 process function defs.
 * stdef.h: Added Win32 def for Bitstream font defines.
 * tcp.c: Added section for Win32 to implement a Windows Sockets interface
 *        for tcp calls.
 *
 * Revision 1.1  1993/05/19  22:04:25  mhd
 * Added Bitstream fontrip (Font Raster Image Processor) c & h files.  Not
 * all are compiles -- some are demos, or test modules.
 *
 * Revision 1.1  1993/05/04  20:15:22  mhd
 * The flat-directory version of the Bitstream 4-in-1 software.
 *
 * Revision 1.2  1993/04/19  15:25:37  mpc
 * Changed local define of _APOLLO from 1 to 0, per bitstream.
 *
...
/* mpc 6-23-93   For now, will make WIN32 defines the same as MSDOS. */
#if defined(MSDOS) || (defined(WIN32) && !defined(BRISTOL)) /*vkp124*/
