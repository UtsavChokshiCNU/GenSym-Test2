...
;;;
;(ext:file-comment
;  "$Header: //gensym/ab-repos/chestnut/src/trans/lisp/loop.lisp,v 1.1 2006/02/28 21:59:08 alatto Exp $")
;;;
;;; **********************************************************************
;;;
;;; $Header: //gensym/ab-repos/chestnut/src/trans/lisp/loop.lisp,v 1.1 2006/02/28 21:59:08 alatto Exp $
...
(himpl:note-rcs-revision
 "$Source: //gensym/ab-repos/chestnut/src/trans/lisp/loop.lisp,v $"
 "$Revision: 1.1 $"
...
(defpackage "TLOOP"
  (:shadowing-import-from "TUTIL" "WARN" "ERROR" "CERROR")
  (:use "HOST" "HOSTX"))
...
(timpl::define-translator-macro loop (&rest stuff)
  "General iteration facility.  See the manual for details, 'cause it's
  very confusing."
  (if (some #'atom stuff)
      (parse-loop stuff)
      `(block nil (timpl::forever ,@stuff))))
...
(defun parse-loop (stuff)
  (let* ((*prologue* nil)
    ...
    (let ((again-tag (make-symbol "AGAIN"))
	  (end-tag (make-symbol "THIS-IS-THE-END")))
      `(block ,name
	 ,(splice-in-subform
	   *outside-bindings*
...
(defun parse-on-for-as (name type)
  (let* ((temp (if (atom name) name (make-symbol "LIST")))
	 (initer (pop *remaining-stuff*))
	 (stepper (if (preposition-p "BY")
		      `(funcall ,(pop *remaining-stuff*) ,temp)
		      `(cdr ,temp))))
...
(defun parse-across-for-as (name type)
  (let* ((temp (make-symbol "VECTOR"))
	 (length (make-symbol "LENGTH"))
	 (index (make-symbol "INDEX")))
    (queue-var *for-as-vars* temp '(vector *) ; was `(vector ,type)
	       :initer (pop *remaining-stuff*))
  ...
  (let ((clause (pop *remaining-stuff*)))
    (cond ((loop-keyword-p clause "HASH-KEY" "HASH-KEYS"
	   ...
	   (let ((table (pop *remaining-stuff*))
		 (iterator (make-symbol (format nil "~A-ITERATOR" name)))
		 (exists-temp (make-symbol (format nil "~A-EXISTS-TEMP" name)))
		 (key-temp (make-symbol (format nil "~A-KEY-TEMP" name)))
		 (value-temp (make-symbol (format nil "~A-VALUE-TEMP" name))))
	     (setf *outside-bindings*
		   (splice-in-subform
		    *outside-bindings*
	  ...
	  ((loop-keyword-p clause "SYMBOL" "PRESENT-SYMBOL" "EXTERNAL-SYMBOL"
	   ...
	   (let ((package
		  (if (or (preposition-p "IN")
		      ...
		      '*package*))
		 (iterator
		  (make-symbol (format nil "~A-ITERATOR" name)))
		 (exists-temp
		  (make-symbol (format nil "~A-EXISTS-TEMP" name)))
		 (symbol-temp
		  (make-symbol (format nil "~A-SYMBOL-TEMP" name))))
	     (setf *outside-bindings*
		   (splice-in-subform
		    *outside-bindings*
