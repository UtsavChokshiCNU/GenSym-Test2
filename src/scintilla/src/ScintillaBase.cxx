...
void ScintillaBase::AutoCompleteCompleted() {
	...
	if(listType == 0) {
	  scn.nmhdr.code = SCN_AFTERAUTOCSELECTION;
	  NotifyParent(scn);
	}
}
...
void ScintillaBase::CallTipShow(Point pt, const char *defn) {
	...
	if (rc.bottom > rcClient.bottom) {
		...
		rc.top -= offset;
		rc.bottom -= offset;
		//The change for remember whether the calltip is at above of the text
		ct.SetIsAboveText(true);  
	}
	else
		ct.SetIsAboveText(false);
...
sptr_t ScintillaBase::WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
	...
	case SCI_CALLTIPSETHLT:
		...
		//only accept the number of the argument signatures that need highlight
		//ct.SetHighlight(wParam, lParam);
        ct.SetHighlight(wParam);
		//}}
		break;
