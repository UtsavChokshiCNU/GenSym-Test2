...
// GENSYMCID-844:
// This variable hold a global variable to limit the undo steps(G2 has a default value 100). 
extern int maxUndoSteps = 100;
...
bool UndoHistory::CanUndo() const {
    int undo_steps = 0;
    for (int i = currentAction; i < maxAction; i++) {
      if (actions[i].at == startAction) {
        undo_steps++;
      }
    }
    return (currentAction > 0) && (maxAction > 0)
      && undo_steps < maxUndoSteps;
}
