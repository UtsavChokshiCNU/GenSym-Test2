...
*/
public class ConfigureDialog extends JDialog implements ObjectChangeListener {
	...
	*/
	public void setAttributeValues(Sequence avalues) {
		...
		Trace.send(Trace.ALL_LEVELS, traceKey, i18n, "SettingAttributes", attributesStructure);
		initializeEditors();
		fireObjectUpdate();
	}
	...
	*/
	...
	public boolean editorsAreValid() {
		for (Iterator i=editors.iterator();i.hasNext();) {
			...
			if (!editor.isValid()) {
				Trace.send(Trace.ALL_LEVELS, traceKey, i18n, "InvalidValueForAttribute", editor.getAttribute());
	...
	*/
	...
	public Sequence getResultStructures() {
		...
		Trace.send(Trace.ALL_LEVELS, traceKey, i18n, "ReturnValuesToApply", attributes);
  ...
  protected JPanel centerPane;
	...
	/** The dialog title will be set to commonName if the name of editedItem is set to none, 
	...
	static MessageKey traceKey = Trace.registerMessageKey("com.gensym.gda", ConfigureDialog.class);
