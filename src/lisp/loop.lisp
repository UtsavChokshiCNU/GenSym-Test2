;;;   LOOP  -*- Mode: Lisp; Package: AB; Base: 10; Lowercase:T; Syntax: Common-lisp -*-
(in-package "AB")
;;;                        ^                                 ^
;;;                        |                                 |
;;;                         -- by mhd; was: Package: LOOP    |
;;;                                                           ---- by mhd; for Symbolics' sake
;;;
;;;
;;;   **********************************************************************
...
(defmacro loop-gentemp (&optional (pref ''loopvar-))
  `(make-symbol
     (format
       nil
       (if (member :translator *features* :test #'eq)
	   "AB-~a"
	   "AB-~a~d")
       (symbol-name ,pref)
...
;;; Since loop often emits dead code, the macro `loop-tagbody' is being
;;; implemented to replace tagbody within expansions produced by loop.  This
;;; macro will eliminate any obviously dead code, and return a call to tagbody
;;; as its expansion.

;;; Often within loop macro expansions, there is obvious dead code which is
;;; being warned about by the C compilers downstream from the Lisp to C
;;; translator.  By "obvious dead code" I mean all forms between a go call as a
;;; top level form in the body of this macro and the next tag.  Go calls in this
;;; macro are forms that are a cons and whose car is either go or loop-finish.
;;; Loop-finish is a LOOP defined, user accessible macro that expands directly
;;; into a go.

;;; This section added by jra 8/10/93.

(defmacro loop-tagbody (&body tags-and-forms)
  (do* ((tagbody-expansion (list 'tagbody))
	(tagbody-tail tagbody-expansion)
	(within-dead-code? nil)
	(tag-or-form-cons? tags-and-forms (cdr tag-or-form-cons?))
	(tag-or-form (car tag-or-form-cons?) (car tag-or-form-cons?)))
       ((null tag-or-form-cons?)
	tagbody-expansion)
    (cond (within-dead-code?
	   (when (or (symbolp tag-or-form) (integerp tag-or-form))
	     (setf (cdr tagbody-tail) (list tag-or-form))
	     (setq tagbody-tail (cdr tagbody-tail))
	     (setq within-dead-code? nil)))
	  (t
	   (setf (cdr tagbody-tail) (list tag-or-form))
	   (setq tagbody-tail (cdr tagbody-tail))
	   (when (and (consp tag-or-form)
		      (or (eq (car tag-or-form) 'go)
			  (eq (car tag-or-form) 'loop-finish)))
	     (setq within-dead-code? t))))))

;; The following is a test case of loop-tagbody.
  ...
  (cond ((null name)
	...
	((atom name)
	   (cond (loop-iteration-variablep
			...
			(if (eq name 'scope-conses)
			    nil ; to enable duplicated SCOPE-CONSES
			  (loop-simple-error
	  ...
	  ((symbolp (car x))
	     (let ((fn (car x)) (tem nil))
	       (cond ((setq tem (get fn 'loop-simplep))
			    ...
			    (setq z (ab-lisp::funcall tem x) x nil)))
    ...
    (let ((stepper (cond ((loop-tequal (car loop-source-code) 'by)
	  ...
	  (var1 nil) (stepvar nil) (step nil) (et nil) (pseudo nil))
       (setq step (if (or (atom stepper)
		      ...
		      `(ab-lisp::funcall ,(setq stepvar (loop-gentemp 'loop-fn-)))
		      (list (cadr stepper))))
       (cond ((and (atom var)
		   ;; (eq (car step) 'cdr)
		   (not fn))
     ...
     (cond ((not (null tem)))
	   ((not (setq tem (loop-tassoc 'default-loop-path
	      ...
	      (loop-simple-error "Undefined LOOP iteration path"
				 (cadr attachment))))
     (setq tem (ab-lisp::funcall (cadr tem) (car tem) var data-type?
