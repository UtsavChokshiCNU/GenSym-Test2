  ...
  (save-excursion
	...
	(let ((next-from (point))) 
	  ...
	  (cond
	    ...
	    (t				; Emacs-Lisp mode
	     ;; defaults to lisp mode behavior, because Schemers can make use
	     ;; of it easily too.  But written with a "cond" instead of "if",
	     ;; for future extensibility.
	     (progn
	       ;; necessary because we don't know if the regexp was "^(def",
	       ;; or maybe "^(defun", or perhaps "^(defun "
	       (backward-sexp)
	       (forward-sexp)
	       (forward-char 1)

	       ;; Look at just the car of (xxx ...) defnames.
	       ;; -fmw, 3/17/95
	       (if (looking-at "(")
		   (forward-char 1))
