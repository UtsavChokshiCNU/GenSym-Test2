...
#ifdef CHESTNUT
/* Need runtime.h for Object, NIL, and error */
#include "runtime.h"
...
extern Object SI_allocate_bignum(/*SI_Dimension length*/);
Object SI_alloc_bignum (length)
    SI_Dimension length;
{
    return SI_allocate_bignum(length);
}
...
#endif
static bignum_type bignum_unsigned_ash();
static bignum_type bignum_unsigned_logand();
static bignum_type bignum_unsigned_logandc2();
static bignum_type bignum_unsigned_logior();
     ...
     SI_Long n;
{
  ...
  {
    fast SI_Ulong accumulator = ((negative_p = (n < 0)) ? (-n) : n);
    do
      {
  ...
  {
    bignum_length_type bignum_length =
      (bignum_length_type)(end_digits - result_digits);
    bignum_type result = (bignum_allocate (bignum_length, negative_p));
...
{
  int negative_p;
  bignum_digit_type result_digits [BIGNUM_DIGITS_FOR_INT64];
  fast bignum_digit_type * end_digits = result_digits;

  /* Special cases win when these small constants are cached. */
  if (n == 0L) return (BIGNUM_ZERO ());
  if (n == 1L) return (BIGNUM_ONE (0));
  if (n == -1L) return (BIGNUM_ONE (1));
  {
    fast SI_uint64 accumulator = ((negative_p = (n < 0L)) ? (-n) : n);
    do
      {
	(*end_digits++) = (accumulator & BIGNUM_DIGIT_MASK);
	accumulator >>= BIGNUM_DIGIT_LENGTH;
  ...
  {
    bignum_type result =
      (bignum_allocate ((bignum_length_type)(end_digits - result_digits), negative_p));
    fast bignum_digit_type * scan_digits = result_digits;
    fast bignum_digit_type * scan_result = (BIGNUM_START_PTR (result));
    while (scan_digits < end_digits)
      (*scan_result++) = (*scan_digits++);
...
/* Added by jar 2-19-91 */
bignum_type
unsigned_long_to_bignum (n)
     SI_Ulong n;
{
  bignum_digit_type result_digits [BIGNUM_DIGITS_FOR_LONG];
  fast bignum_digit_type * end_digits = result_digits;
  ...
  {
    bignum_type result =
      (bignum_allocate ((bignum_length_type)(end_digits - result_digits), 0));
    fast bignum_digit_type * scan_digits = result_digits;
    fast bignum_digit_type * scan_result = (BIGNUM_START_PTR (result));
    while (scan_digits < end_digits)
      (*scan_result++) = (*scan_digits++);
     ...
     bignum_type bignum;
{
    ...
    return 0;
  {
    bignum_length_type index = ((BIGNUM_LENGTH (bignum)) - 1);
    fast bignum_digit_type digit = (BIGNUM_REF (bignum, index));
    fast SI_Long result = index * BIGNUM_DIGIT_LENGTH;
    while (digit > 0)
      {
	++result;
	digit >>= 1;
...
{
  fast bignum_type result = (bignum_allocate ((bignum_length_type)2, 0));
     ...
     int negative_p;
{
  ...
  if (n_digits == 1)
    {
      SI_Long digit = ((SI_Long) ((*producer) (context)));
  ...
  if (! (BIGNUM_ZERO_P (bignum)))
      ...
      while (start < scan)
	{
	  if ((scan[-1]) == 0)
	    ...
	    (*consumer)
	      (context, (bignum_destructive_scale_down (working_copy, (bignum_digit_type)radix)));
	}
     ...
     int negative_p;
{
  if (digit == 0)
    ...
    {
      fast bignum_type result = (bignum_allocate ((bignum_length_type)1, negative_p));
     ...
     bignum_type bignum;
{
  ...
  if (BIGNUM_ZERO_P (bignum)) return (bignum);
  ...
  if (scan < end)
    {
      fast bignum_length_type length = (bignum_length_type)(scan - start);
      BIGNUM_SET_HEADER
	(bignum, length, ((length != 0) && (BIGNUM_NEGATIVE_P (bignum))));
      BIGNUM_REDUCE_LENGTH (bignum, bignum, length);
    }
...

{
 ...
 unboxed_n = IFIX(n);
 result =  bignum_unsigned_ash (arg1, unboxed_n);

 return((BIGNUM_NEGATIVE_P (arg1)) 
        ? (unboxed_n < 0)
 	  ? bignum_lognot (bignum_unsigned_ash (bignum_lognot (arg1), unboxed_n))
   	  : bignum_subtract (BIGNUM_ZERO (), result)
	: result);
}

SI_Long
bignum_integer_length(arg1)
     bignum_type arg1;
{
 return((BIGNUM_NEGATIVE_P (arg1)) 
        ? bignum_length_in_bits (bignum_lognot (arg1))
	: bignum_length_in_bits (arg1));
}

Object
bignum_logcount(arg1)
     bignum_type arg1;
{
 return(FIX((BIGNUM_NEGATIVE_P (arg1)) 
	    ? bignum_unsigned_logcount (bignum_lognot (arg1))
	    : bignum_unsigned_logcount (arg1)));
}


bignum_type
bignum_logand(arg1,arg2)
     bignum_type arg1, arg2;
{
  return(
	 (BIGNUM_NEGATIVE_P (arg1))
	 ? (BIGNUM_NEGATIVE_P (arg2))
	   ? bignum_lognot (bignum_unsigned_logior
			 (bignum_lognot (arg1),bignum_lognot (arg2)))
       	   : bignum_unsigned_logandc2 (arg2,bignum_lognot (arg1))
	 : (BIGNUM_NEGATIVE_P (arg2))
	   ? bignum_unsigned_logandc2 (arg1,bignum_lognot (arg2))
	   : bignum_unsigned_logand (arg1,arg2)
	 );
}

bignum_type
bignum_logior(arg1,arg2)
  bignum_type arg1, arg2;
{
  return(
	 (BIGNUM_NEGATIVE_P (arg1))
	 ? (BIGNUM_NEGATIVE_P (arg2))
	   ? bignum_lognot (bignum_unsigned_logand
			 (bignum_lognot (arg1),bignum_lognot (arg2)))
	   : bignum_lognot (bignum_unsigned_logandc2 (
				 bignum_lognot (arg1),arg2))
	 : (BIGNUM_NEGATIVE_P (arg2))
	   ? bignum_lognot (bignum_unsigned_logandc2 (
				 bignum_lognot (arg2),arg1))
	   : bignum_unsigned_logior (arg1,arg2)
	 );
}

/* ash for positive big numbers */
/* assume arg1 is a big number, n is a long */
static bignum_type
bignum_unsigned_ash(arg1,n)
  bignum_type arg1;
  long n;
{
  bignum_type result;
  fast bignum_digit_type *scan1;
  fast bignum_digit_type *scanr;
  fast bignum_digit_type *end;

  int digit_offset,bit_offset;
  ...
  if (n > 0) {
    ...
    result = bignum_allocate_zeroed (BIGNUM_LENGTH (arg1)
				     + digit_offset + 1,0);
    ...
    result = bignum_allocate_zeroed (BIGNUM_LENGTH (arg1) - digit_offset,0);
...
/* all functions below are implemented with an assumption that
   arguments arg1 and arg2 are big numbers */
...
{
  bignum_type result;
  bignum_length_type min_length;

  fast bignum_digit_type *scan1;
  fast bignum_digit_type *scan2;
  fast bignum_digit_type *scanr;
  fast bignum_digit_type *end;

  if (BIGNUM_ZERO_P (arg1)) return (arg1);
  if (BIGNUM_ZERO_P (arg2)) return (arg2);

  min_length =  (BIGNUM_LENGTH (arg1) < BIGNUM_LENGTH (arg2))
               ? BIGNUM_LENGTH (arg1) : BIGNUM_LENGTH (arg2);

  result = bignum_allocate_zeroed (min_length,0);

  scanr = BIGNUM_START_PTR (result);
  scan1 = BIGNUM_START_PTR (arg1);
  scan2 = BIGNUM_START_PTR (arg2);
  end = scan1 + min_length;
...
/* unsigned inclusive or */ 
static bignum_type
bignum_unsigned_logior (arg1, arg2)
     bignum_type arg1, arg2;
{
  bignum_type result;
  bignum_length_type min_length, max_length;
  ...
  if (BIGNUM_ZERO_P (arg1)) return (arg2);
  if (BIGNUM_ZERO_P (arg2)) return (arg1);
  ...
  result = bignum_allocate_zeroed (max_length,0);
  ...
  if (BIGNUM_LENGTH (arg1) > min_length) {
    end = BIGNUM_START_PTR (arg1) + BIGNUM_LENGTH (arg1);
    while (scan1 < end )
      *scanr++ = *scan1++;
  }

  else if (BIGNUM_LENGTH (arg2) > min_length) {
    end = BIGNUM_START_PTR (arg2) + BIGNUM_LENGTH (arg2);
    while (scan2 < end )
      *scanr++ =  *scan2++;
  }
...
/* unsigned logandc2 */
static bignum_type
bignum_unsigned_logandc2(arg1, arg2)
     bignum_type arg1,arg2;
{
  bignum_type result;
  bignum_length_type min_length;
  ...
  if (BIGNUM_ZERO_P (arg1) || BIGNUM_ZERO_P (arg2)) return (arg1);

  min_length =  (BIGNUM_LENGTH (arg1) < BIGNUM_LENGTH (arg2))
               ? BIGNUM_LENGTH (arg1) : BIGNUM_LENGTH (arg2);

  result = bignum_allocate_zeroed (BIGNUM_LENGTH(arg1),0);
...
static long
bignum_unsigned_logcount(arg)
  bignum_type arg;
{

  fast bignum_digit_type *scan;
  fast bignum_digit_type *end;
  fast bignum_digit_type digit;

  /* suffisient for any reasonable big number */
  long result;
  int i;
...
/* We can speed these up by having transforms, so that the
   translator can emit the macro directly.
   */

#ifdef CHESTNUT
Object bignum_length(x)
  Object x;
{
  return XBIGNUM_LENGTH(x);
}

Object copy_of_bignum(x)
  Object x;
{
  return bignum_copy(x);
}

Object store_bignum(source, target)
  Object source, target;
{
  if (BIGNUM_LENGTH(source) == BIGNUM_LENGTH(target)) {
    bignum_destructive_copy(source, target);
    return(Qt);
  } else 
    return Qnil;
