/*
 * substfnt.c -  Chestnut Run-Time Library entry for function subst_if_not
 *
...
Object subst_if_not (new, test, tree, key)
    Object new, test, tree, key;
{
	...
	a = subst_if_not(new,test,M_CAR(tree),key);
	d = subst_if_not(new,test,M_CDR(tree),key);
