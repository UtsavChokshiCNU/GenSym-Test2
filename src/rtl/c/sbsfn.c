/*
 * sbsfn.c -  Chestnut Run-Time Library entry for function substitute_if_not
 *
...
Object substitute_if_not (newitem, test, seq, from_end, start, end,
			  count, key)
    Object newitem, test, seq, from_end, start, end, count, key;
{
    ...
    seq = copy_seq(seq);
    RESTORE_STACK();
    return(nsubstitute_if_not(newitem,test,seq,from_end,
		       start,end,count,key));
}
