/*
 * sfaref.c -  Chestnut Run-Time Library entry for function set_foreign_aref
 *
...
Object set_foreign_aref varargs_1(int, n)
{
    Object new_value, foreign_pointer;
    ...
    if(!(FOREIGN_POINTER_TYPE_P(foreign_pointer) &&
	 ...
	 ARRAY_FOREIGN_TYPE_P(atype =
      ...
      error(1,"Illegal call to set_foreign_aref");
    dims_tail = ARRAY_FOREIGN_TYPE_DIMENSIONS(atype);
    while (CONSP(dims_tail)) {
      if(!CONSP(indices))
	error(1,"set_foreign_aref: incorrect number of indices supplied");
      inds[ndims] = ind = IFIX(M_CAR(indices));
      dims[ndims] = dim = IFIX(M_CAR(dims_tail));
      if ( (ind < 0) || (ind >= dim))
	error(4,"set_foreign_aref: Bounds error: dim# ~D, index ~D, dim ~D",
