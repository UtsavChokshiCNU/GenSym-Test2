...
typedef SI_Bignum_digit_type bignum_digit_type;
typedef SI_Long bignum_length_type;
...
/* defs for chestnut are in object.h */
...
#define BIGNUM_SET_HEADER(bignum, length, negative_p)			\
  (BIGNUM_HEADER (bignum) =						\
    ((length) | ((negative_p) ? (-1L << BIGNUM_DIGIT_LENGTH) : 0)))

#define BIGNUM_LENGTH(bignum)						\
  ((BIGNUM_HEADER (bignum)) & BIGNUM_DIGIT_MASK)

#define BIGNUM_NEGATIVE_P(bignum)					\
  (((BIGNUM_HEADER (bignum)) & BIGNUM_RADIX) != 0)

#endif


#ifndef BIGNUM_DIGIT_LENGTH /* might be preset in config.h */
#define BIGNUM_DIGIT_LENGTH (((sizeof (bignum_digit_type)) * CHAR_BIT) - 2)
#endif

#define BIGNUM_HALF_DIGIT_LENGTH (BIGNUM_DIGIT_LENGTH / 2)
#define BIGNUM_RADIX		 ((SI_Long)1L << BIGNUM_DIGIT_LENGTH)
#define BIGNUM_RADIX_ROOT	 ((SI_Long)1L << BIGNUM_HALF_DIGIT_LENGTH)
#define BIGNUM_DIGIT_MASK	 (BIGNUM_RADIX - 1L)
#define BIGNUM_HALF_DIGIT_MASK	 (BIGNUM_RADIX_ROOT - 1L)

#define BIGNUM_ZERO_P(bignum)						\
  ((BIGNUM_LENGTH (bignum)) == 0L)
...
extern Object SI_static_bignum_zero;
extern Object SI_static_bignum_one;
extern Object SI_static_bignum_negative_one;

#define BIGNUM_ZERO()	SI_static_bignum_zero
#define BIGNUM_ONE(negativep) \
    (negativep ? SI_static_bignum_negative_one : SI_static_bignum_one)
#else
...
#define BIGNUM_BITS_TO_DIGITS(n)					\
  (((n) + (BIGNUM_DIGIT_LENGTH - 1L)) / BIGNUM_DIGIT_LENGTH)

#define BIGNUM_DIGITS_FOR_LONG						\
  (BIGNUM_BITS_TO_DIGITS ((sizeof (SI_Long)) * CHAR_BIT))

#define BIGNUM_DIGITS_FOR_INT64						\
  (BIGNUM_BITS_TO_DIGITS ((sizeof (SI_int64)) * CHAR_BIT))
