/*
 * nsbstfnt.c -  Chestnut Run-Time Library entry for function nsubst_if_not
 *
...
Object nsubst_if_not (new, test, tree, key)
    Object new, test, tree, key;
{
	...
	treeitem = nsubst_if_not(new, test, M_CAR(tree), key);
	RPLACA(tree, treeitem);
	treeitem = nsubst_if_not(new, test, M_CDR(tree), key);
