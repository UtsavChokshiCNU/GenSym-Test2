/*
 * stfaref.c -  Chestnut Run-Time Library entry for function set_typed_foreign_aref
 *
...
extern Object SI_make_rest_arg(Object *args, long n);
...
Object set_typed_foreign_aref varargs_1(int, n)
{
    Object new_value, foreign_type, foreign_pointer;
    ...
    SAVE_STACK();
    INIT_ARGS();
    new_value = REQUIRED_ARG();
    foreign_type = REQUIRED_ARG();
    foreign_pointer = REQUIRED_ARG();
    SAVE_ARGS();
    indices = REST_ARG();
    PROTECT_4(new_value,foreign_type,foreign_pointer,indices);
	...
	error(1,"foreign_aref: incorrect number of indices supplied");
      ...
      if ( (ind < 0) || (ind >= dim))
	error(4,"set_typed_foreign_aref: Bounds error: dim# ~D, index ~D, dim ~D",
    ...
    if(foreign_type != type)
      error(1,"set_typed_foreign_aref: not of required type");
    RESTORE_STACK();
    return 
