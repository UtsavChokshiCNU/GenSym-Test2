...
set TITLE_="G2 Server"
set HM=%SITE_%
set G2_PATH=%HM%\g2
if "%G2_EXEC%" == "" set G2_EXEC=g2

:start

set PATH=%HM%\bin;%HM%\javalink\bin;%HM%\tw2\bin;%HM%\gsi;%HM%\activexlink\bin;%HM%\odbc\bin;%HM%\oracle\bin;%HM%\sybase\bin;%HM%\pi\bin;%HM%\opcclient;%HM%\g2i\bin;%HM%\corbalink;%HM%\g2;%HM%\jre\bin;%HM%\g2\x64;%PATH%;
...
if exist "%HM%\opcclient\kbs" set G2_MODULE_SEARCH_PATH=%G2_MODULE_SEARCH_PATH% '%HM%\opcclient\kbs'
...
if exist "%HM%\jsnmp\kbs" set G2_MODULE_SEARCH_PATH=%G2_MODULE_SEARCH_PATH% '%HM%\jsnmp\kbs'
...
if exist "%HM%\hla\kbs" set G2_MODULE_SEARCH_PATH=%G2_MODULE_SEARCH_PATH% '%HM%\hla\kbs'
...
start %TITLE_% /b "%G2_PATH%\%G2_EXEC%" %1 %2 %3 %4 %5 %6 %7 %8 %9 -resourcefile '%SITE_%\g2i\kbs\resources-*.txt' -configfile '%SITE_%\g2i\kbs\config.txt'
