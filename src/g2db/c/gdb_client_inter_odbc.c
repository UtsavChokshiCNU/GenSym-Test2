/*================================================================
 * GDB_CLIENT_INTER_ODBC.C  - GDB Layer-3 (ODBC 3.0)  [source]
 *================================================================
 *
 * Author: Peter Hill
 *
...
long get_genda_data(GENDA *sqlGen)
{
    ...
    */
    ...
    data_ptr = (GDB_CHAR**)calloc(select_ptr[0].ncols, sizeof(GDB_CHAR*)) ;
    ...
    {    
        for(col=0;col<select_ptr[0].ncols;col++)
        {
            ...
            switch (select_ptr[col].datatype)
            {
                ...
                case SQL_GUID:              
                    ...
                    if ((sqlGen->da_col_ptr[col].val[row].gdb_str = 
                                                (GDB_CHAR*)calloc(select_ptr[col].datalen+1, sizeof(GDB_CHAR))) == NULL) 
                                            {
                        ...
                        return ( GDB_ERROR );
                                            ...
                                            wcsncpy(sqlGen->da_col_ptr[col].val[row].gdb_str, (GDB_CHAR*)(data_ptr[col]+start_pos[col]),
                             select_ptr[col].datalen );
                                            sqlGen->da_col_ptr[col].val[row].gdb_str[select_ptr[col].datalen] = 0 ;
                    start_pos[col] += select_ptr[col].datalen+1;
                    if ( ( (select_ptr[col].ind[in_row]== SQL_NULL_DATA) &&
                           (in_row < last_row) ) ||
                                                (sqlGen->genStat.code == end_of_cursor) ) 
                                            {
                                                if ((sqlGen->da_col_ptr[col].val[row].gdb_str = (GDB_CHAR*)realloc(sqlGen->da_col_ptr[col].val[row].gdb_str,
                ...
                case SQL_INTEGER :
                    ...
                    if ( (select_ptr[col].ind[in_row] == SQL_NULL_DATA) ||
                        ...
                        sqlGen->da_col_ptr[col].val[row].gdb_long = *((int *)data_ptr[col]);

                    sqlGen->da_col_ptr[col].ind[row] = select_ptr[col].ind[row];
                    pint_temp = (int *)data_ptr[col];
                    pint_temp++;
                    data_ptr[col] = (GDB_CHAR*)pint_temp;
                break; 

                case SQL_SMALLINT :
                case SQL_TINYINT :
            case SQL_BIT :                  sqlGen->da_col_ptr[col].data_type = GDB_SHORT_TYPE ;
            ...
            case SQL_REAL :                 sqlGen->da_col_ptr[col].data_type = GDB_FLOAT_TYPE ;
                    ...
                    sqlGen->da_col_ptr[col].ind[row] = select_ptr[col].ind[row];
                    pfloat_temp = (float *)data_ptr[col];
                    pfloat_temp++;
                    data_ptr[col] = (GDB_CHAR*)pfloat_temp;

                break;

                case SQL_FLOAT :
            case SQL_DOUBLE :               sqlGen->da_col_ptr[col].data_type = GDB_DBL_TYPE ;
                    if ( (select_ptr[col].ind[in_row] == SQL_NULL_DATA) ||
                         (sqlGen->genStat.code == end_of_cursor) )
                        sqlGen->da_col_ptr[col].val[row].gdb_dbl = context[context_num].null_number;
                    else
                    ...
                    sqlGen->da_col_ptr[col].ind[row] = select_ptr[col].ind[row];
                    pdouble_temp = (double *)data_ptr[col];
                    pdouble_temp++;
                    data_ptr[col] = (GDB_CHAR*)pdouble_temp;

                break;

                case SQL_NUMERIC :
            case SQL_DECIMAL :              /* Field encompasses sign, data, and null (i.e., length = precision + 2) */
                    /* --------------------------------------------------------------------- */
                    tlen = select_ptr[col].datalen + select_ptr[col].decdigs + 2;

                                            if ((temp_str = (GDB_CHAR*)calloc(tlen, sizeof(GDB_CHAR))) == NULL) 
                                            {
                                                iomsg(DBG6|DBG9, LAYER, TRUE, FALSE, NULL, FALSE, "%s", _T("Could NOT alloc gdb_str buffers")) ;
                                                iomsg(DBGM, LAYER, TRUE, TRUE, sqlGen, TRUE, "%d", EINSMEM) ;
                        free( data_ptr );                        
                    ...
                    if ( (select_ptr[col].ind[in_row]== SQL_NULL_DATA) ||
                        ...
                        sqlGen->da_col_ptr[col].val[row].gdb_dbl = context[context_num].null_number;
                                            ...
                                            {
                                                temp_str_len = wcslen(temp_str) ;
                                                pNarrow = calloc(temp_str_len+1, sizeof(char)) ;
                                                for (lngX = 0 ; lngX < temp_str_len ; lngX++)
                                                    pNarrow[lngX] = (char)temp_str[lngX] ;
                        ...
                        /* ------------------------------------------------------------------------ */
                            ...
                            sqlGen->da_col_ptr[col].data_type = GDB_INT_TYPE;
                                                    sqlGen->da_col_ptr[col].val[row].gdb_int = atoi(pNarrow) ;
                        }
                        ...
                        /* -------------------------------------------------- */
                            ...
                            sqlGen->da_col_ptr[col].data_type = GDB_DBL_TYPE;
                                                    sqlGen->da_col_ptr[col].val[row].gdb_dbl = atof(pNarrow) ;
                        }
                    }
                    sqlGen->da_col_ptr[col].ind[row] = select_ptr[col].ind[row];
                    free( temp_str );
...
long db_alloc_genda(GENDA *sqlGen, long max_rows)
{
  ...
  select_ptr = sqlda_ptrs[context_num][sqlGen->stmt_handle].sptr;
    ...
    /* allocate enough bytes for column name and set the column name in genda */
        if ((sqlGen->da_col_ptr[col].col_name = (GDB_CHAR*)calloc(name_len+1, sizeof(GDB_CHAR))) == NULL) 
        {
    ...
    /** alloc max_rows val buffers **/
        ...
        case SQL_TYPE_TIME :
        case SQL_TIMESTAMP :
            case SQL_GUID:          sqlGen->da_col_ptr[col].data_type = GDB_STRING_TYPE ;
            break;  

        case SQL_WCHAR:
        case SQL_VARCHAR :
        case SQL_LONGVARCHAR :
            sqlGen->da_col_ptr[col].data_type = GDB_WSTRING_TYPE;
            break;
        ...
        case SQL_DOUBLE :
        case SQL_NUMERIC :
            case SQL_DECIMAL :      sqlGen->da_col_ptr[col].data_type = GDB_DBL_TYPE ;
            break;
    }
  } /* end for loop */
...
long get_stat(GENDA *sqlGen)
{
    ...
    GDB_CHAR text_msg[MAX_ODBC_MSG_LEN+1] ;
    GDB_CHAR ss_msg[10] ;
  long   msg_len  = 0;
  long   warn_len = 0;
  long   prefix_len = 0;
  ...
  context_num = gsi_current_context();
    ...
    {
        if ((context[context_num].gbl_status.msg = (GDB_CHAR*)calloc(MAX_ODBC_MSG_LEN+1, sizeof(GDB_CHAR))) == NULL) 
        {
  ...
  /* set the message length and the text_msg if there's an error */
    ...
    /* loop to add each ODBC error message to the text message */
      ...
      */
	  notDSN = wcscmp(_T("<dsn>"), g_sqlca.state_array[i].sqlstate) ? 1 : 0;
      ...
      {
        numerrs++;
	if (!wcscmp(_T("<dsn>"), g_sqlca.state_array[i].sqlstate))
                    _snwprintf(ss_msg, 10, _T("%s"), !i?_T(""):_T("; ")) ;
        else
                    _snwprintf(ss_msg, 10, _T("%sss=%5s "), !i?_T(""):_T("; "), g_sqlca.state_array[i].sqlstate) ;
        if (i==0)
  ...
  */
  if ( (msg_len) ||
    ...
    /* concat prefix and message body */
        _snwprintf(context[context_num].gbl_status.msg, MAX_ODBC_MSG_LEN,_T("%s%s"), prefix_msg, text_msg) ;
  }
  ...
  /* load the GENDA struct */
    ...
    if ( sqlGen->more_rows >= sqlGen->nrows )
      sqlGen->more_rows = 0;
        ...
        {
            if ((sqlGen->genStat.msg = (GDB_CHAR*)calloc(msg_len+1, sizeof(GDB_CHAR))) == NULL) 
            {
	...
	if( g_sqlca.code == GDB_BIND_ERROR )
	{
		...
		sqlGen->genStat.msg = (GDB_CHAR *)realloc(sqlGen->genStat.msg,(msg_len+1)*sizeof(GDB_CHAR));
  ...
  wcscpy(g_sqlca.state_array[0].native_errmsg, _T("")) ;
  ...
  { 
    ...
    wprintf(_T("msg   : %s\n"),context[context_num].gbl_status.msg); /* msg is GDB_CHAR* */
...
/*======================================================================
  load_bind_vars
  ======================================================================*/
long load_bind_vars(long shandle, GDB_CHAR **bind_vars, long index, long datatype, long array_size)
{
    gSQLDA     *bind_ptr;
    ...
    */
    switch ( bind_ptr[index].datatype )
    {
        ...
        case SQL_TYPE_TIME :
        case SQL_TYPE_TIMESTAMP :
        case SQL_GUID:              if (datatype != GDB_STRING_TYPE) 
                                    {
        ...
        case SQL_SMALLINT :
        case SQL_TINYINT :
        case SQL_BIT :              if (datatype != GDB_INT_TYPE) 
                                    {
                                        iomsg(DBG3, LAYER, TRUE, TRUE, NULL, FALSE, "%d%d", EINCDTP, index+1) ;
                return( GDB_ERROR );
            }
        ...
        case SQL_FLOAT :
        case SQL_NUMERIC :
        case SQL_DECIMAL :          if (datatype != GDB_FLOAT_TYPE) 
                                    {
                                        iomsg(DBG3, LAYER, TRUE, TRUE, NULL, FALSE, "%d%d", EINCDTP, index+1) ;
                return( GDB_ERROR );
            }
    ...
    if ((ind_array = (long *)calloc(array_size, sizeof(long))) == NULL) 
    {
    ...
    switch(datatype) 
    {
        case GDB_INT_TYPE:      elmntsize = (long)sizeof(long) ;
            if ((bind_ptr[index].bdata = 
                                    (GDB_CHAR*)calloc(array_size+1, elmntsize)) == NULL) 
                                {
                ...
                return (GDB_ERROR);
                                ...
                                if ((bind_ptr[index].ind = (long *)calloc(array_size+1, sizeof(long))) == NULL) 
                                {
        ...
        case GDB_FLOAT_TYPE:    elmntsize = (long)sizeof(double) ;
            if ((bind_ptr[index].bdata = 
                                    (GDB_CHAR*)calloc(array_size+1, elmntsize)) == NULL) 
                                {
                                    iomsg(DBG6|DBG9, LAYER, TRUE, FALSE, NULL, FALSE, "%s", _T("Could not alloc data array")) ;
        ...
        case GDB_WSTRING_TYPE:
            elmntsize = (datatype == GDB_WSTRING_TYPE) ? (long)sizeof(wchar_t) : (long)sizeof(char);
            /* calc the total number of chars in the bind_var array */
            len = (bind_ptr[index].colsize+1) * array_size;

            if ((bind_ptr[index].bdata = 
                                    (GDB_CHAR*)calloc(len, elmntsize)) == NULL) 
                                {
                                    iomsg(DBG6|DBG9, LAYER, TRUE, FALSE, NULL, FALSE, "%s", _T("Could not alloc data array")) ;
                                    iomsg(DBGM, LAYER, TRUE, TRUE, NULL, FALSE, "%d", EINSMEM) ;
                                    free(ind_array) ;
            ...
            tptr = bind_ptr[index].bdata;
                ...
                */
                ...
                if ( (context[context_num].set_null_options & 0x2) && 
                    ...
                    ind_array[lX] = SQL_NULL_DATA;
                                    else if ((context[context_num].set_null_options & 0x4) && !_wcsicmp(bind_vars[lX], _T("NULL") )) 
                    ind_array[lX] = SQL_NULL_DATA;
            }
            memcpy(bind_ptr[index].ind, ind_array, array_size*sizeof(long));
            break;
...
long count_bind_vars(long shandle, GDB_CHAR *bind_vars)
{
    register int nBind;
    gSQLDA  *bind_ptr;
    GDB_CHAR bind_name[MAX_VNAME_LEN] ;
    ...
    nBind = 0;
    while (strgettoken(bind_vars, bind_name, _T(','), nBind) != NULL)
        nBind++;
