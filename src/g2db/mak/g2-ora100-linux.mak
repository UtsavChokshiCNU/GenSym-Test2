...
DB_GRP = 10
DBNAME = ORACLE
XDBDIR = ${SRCBOX}/g2db/dbs/$(PLATFM)/oracle/client_10_2
...
g2-ora10:                   settings                            \
                            ...
                            gdb_client_ora10.o
	@cd $(EXEDIR)
	@$(GCC) -L$(XDBDIR)/lib -L${GSIDIR} -L${EXTDIR} -L${RTLDIR} -o g2-ora10 \
		-Wl,--wrap=memcpy ${EXTDIR}/nsleep.o \
		gdb_main.o gdb_g2_server.o gdb_common.o  \
		gdb_utils.o gsi_brg.o gdb_client_inter_ora.o gdb_client_ora10.o  \
...
gdb_client_inter_ora.o:     $(GDBDIR)/gdb_client_inter_ora.c    \
                            ...
                            $(GDBDIR)/gdb_utils.h               \
                            $(GDBDIR)/gdb_common.h
	@$(CC) -c  -I$(GDBDIR) -I$(GSICDIR) -I$(XDBDIR)/precomp/public -D$(DBNAME) $(GDBDIR)/gdb_client_inter_ora.c
	@ls -l gdb_client_inter_ora.o

gdb_client_ora10.o:         gdb_client_ora10.c                  \
                            $(GDBDIR)/gdb_globals.h             \
                            $(GDBDIR)/gdb_client_ora.h          \
                            $(GDBDIR)/gdb_globals.h             \
                            $(GDBDIR)/gdb_messages.h            \
                            ...
                            $(GDBDIR)/gdb_common.h              \
                            $(GDBDIR)/gdb_g2_client.h
	@$(CC) -c -D $(PLATFM) -D iDBVER=$(DB_GRP) -I$(GDBDIR) -I$(GSICDIR) -I$(GDBDIR) -I$(XDBDIR)/precomp/public \
        -DORA09 -DHYBRID -DMAXOPENCURSORS=44 -DHOLD_CURSOR="yes" -DRELEASE_CURSOR="no" -D$(DBNAME) gdb_client_ora10.c
	@ls -l gdb_client_ora10.o

# Precompiler
# -----------
gdb_client_ora10.c:         $(GDBDIR)/gdb_client_ora.pc         \
                            $(GDBDIR)/gdb_client_ora.h          \
                            $(GDBDIR)/gdb_common.h              \
                            $(GDBDIR)/gdb_context.h             \
                            $(GDBDIR)/gdb_g2_client.h           \
	...
	@$(XDBDIR)/bin/proc iname=$(GDBDIR)/gdb_client_ora.pc oname=gdb_client_ora10.c \
        include=$(GDBDIR)  include=$(GSICDIR) include=$(XDBDIR)/precomp/public \
        SQLCHECK=SEMANTICS MAXOPENCURSORS=44 HOLD_CURSOR=yes RELEASE_CURSOR=no
	@ls -l gdb_client_ora10.c
...
clean :
	@- rm *.o 2> /dev/null
	@- rm tp* 2> /dev/null
	@- rm gdb_client_ora10.c 2> /dev/null
