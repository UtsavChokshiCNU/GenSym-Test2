/* ========================================================================= */
/*                                                                           */
/* OPC_GSI_BRG.C - Bridge Licensing Module                                   */
/*                                                                           */
...
/* -------------------------------------------------------------- */
    ...
    extern "C" {
    extern long g2ext_nsleep(char *buffer); /* retrieves machine-id whose format may vary
...
{
    FILE                *fokin;
    char                line[MAXLINE], s[17][MAXLINE], *pLine;
    char                machID[MAXLINE] ;
    ...
    /* -------------------------------------------------- */
#ifdef VS2005
    strcpy_s(ok.prod, MAXLINE, product) ;
    gb_raise_to_uppercase(ok.prod);
    strcpy_s (vers_upper, MAXLINE, version);
#else
    strcpy(ok.prod, product) ;
    gb_raise_to_uppercase(ok.prod);
    strcpy(vers_upper, version);
#endif
    gb_raise_to_uppercase (vers_upper);
    ...
    /* --------------------------------------------------------- */
#ifdef VS2005
    errno = getenv_s(&retSz, line, MAXLINE, OK_LOGICAL) ;
	pLine = line ;
    if (errno)
#else
    pLine = getenv(OK_LOGICAL) ;
	if (pLine == NULL)
#endif
    {
...
#ifdef VS2005
    strcpy_s(ok.fin_dir, MAXLINE, pLine);
#else
    strcpy(ok.fin_dir, pLine);
#endif
    ...
    /* ---------------------------------------------------------------- */
#ifdef VS2005
    strcpy_s(ok.fin, MAXLINE, ok.fin_dir);
    strcat_s(ok.fin, MAXLINE, OK_FILE_LOWERCASE);
    errno = fopen_s(&fokin, ok.fin, "r") ;
    if (errno)
#else
    strcpy(ok.fin, ok.fin_dir);
    strcat(ok.fin, OK_FILE_LOWERCASE);
    fokin = fopen(ok.fin, "r") ;
	if (fokin == NULL)
#endif
    {
    /*                   The bridge is not authorized                  */
    /* --------------------------------------------------------------- */
    ...
    {   
        ...
        /* -------------------------------------------------- */
#ifdef VS2005
        strcpy_s(ok.machine_id, MAXLINE, buffer);
#else
        strcpy(ok.machine_id, buffer);
#endif
        gb_normalize_machine_id(ok.machine_id);
#ifdef AUTH_DEBUG
	printf("Machine ID: %s  (normalized: %s)\n", buffer, ok.machine_id);
        ...
        {
#ifdef VS2005
            strcpy_s(machID, MAXLINE, ok.machine_id) ;
#else
        ...
        while (1)
        {
            ...
            /* --------------------------------------------------------------------*/
#ifdef VS2005
            n = sscanf_s(line, "%*s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s",
                        s[0], MAXLINE, s[1], MAXLINE, s[2], MAXLINE, s[3], MAXLINE,
                        s[4], MAXLINE, s[5], MAXLINE, s[6], MAXLINE, s[7], MAXLINE,
                        s[8], MAXLINE, s[9], MAXLINE, s[10],MAXLINE, s[11],MAXLINE,
                        s[12],MAXLINE, s[13],MAXLINE, s[14],MAXLINE, s[15],MAXLINE,
                        s[16],MAXLINE);
#else
            ...
            {
#ifdef VS2005
                strcpy_s(ok.machine_id,MAXLINE,s[0]);
#else
...
short int verify_month(char *month)
{
...
short int verify_day(short day, short month, short year)
{
...
long int universal_date(short day, short month, short year)
{
  register short int i;
  long int j;
