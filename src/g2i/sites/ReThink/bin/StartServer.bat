...
rem
rem	This batch file launches the ReThink server and load a default solution.
...
set TITLE_="ReThink 4.1r0 Server"
set TCP_EXACT=
set HM=%SITE_%
set G2_PATH=%HM%\g2

set DEF_KB_="%SITE_%\ReThink\kbs\ReThink.kb"
set KB_=%DEF_KB_%
...
set G2_MODULE_SEARCH_PATH=" '%HM%\ReThink\kbs' '%HM%\ReThink\examples' '%HM%\g2i\kbs' '%HM%\jmail\kbs' '%HM%\jms\kbs' '%HM%\g2\kbs\utils' '%HM%\gda' '%HM%\activexlink' '%HM%\odbc\' '%HM%\oracle' '%HM%\pi' '%HM%\sybase' '%HM%\opclink\kbs' '%HM%\protools\kbs' '%HM%\gw' '%HM%\corbalink\kbs' '%HM%\javalink\kbs' '%HM%\tw2\kbs' "
...
start %TITLE_% /b "%G2_PATH%\g2" -background gray %TCP_EXACT% -log "%HM%\ReThink\logs\g2log.txt" -icon %TITLE_% -name %TITLE_% -resourcefile "%SITE_%\g2i\kbs\resources-*.txt" -configfile "%SITE_%\g2i\kbs\config.txt" -kb %KB_%
