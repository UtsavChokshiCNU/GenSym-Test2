 ...
 *      callback files for different systems.
 +   2007/12/16, jv: created g2callbacks.c from twcallbacks.c
 +
 +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
 ...
 * G2INT-X-ERROR
 */
static long (__stdcall *g2int_x_error_call_handle)(long);

DLLEXPORT void g2int_x_error_init_call_handle (long (__stdcall *fn)(long))
{
...
long g2int_x_error(long error_code)
{
  ...
  if (g2int_x_error_call_handle) {
    return_value = (*g2int_x_error_call_handle)(error_code);
  } else {
 ...
 */
static long (__stdcall *g2int_service_sigalrm_signal_call_handle)();

DLLEXPORT void g2int_service_sigalrm_signal_init_call_handle (long (__stdcall *fn)())
{
...
long g2int_service_sigalrm_signal()
{
  ...
  if (g2int_service_sigalrm_signal_call_handle) {
    return_value = (*g2int_service_sigalrm_signal_call_handle)();
  } else {
 ...
 */
static long (__stdcall *g2int_signal_handler_call_handle)();

DLLEXPORT void g2int_signal_handler_init_call_handle (long (__stdcall *fn)())
{
...
long g2int_signal_handler()
{
  ...
  if (g2int_signal_handler_call_handle) {
    return_value = (*g2int_signal_handler_call_handle)();
  } else {
 ...
 */
static long (__stdcall *g2int_signal_handler_for_launch_call_handle)();

DLLEXPORT void g2int_signal_handler_for_launch_init_call_handle (long (__stdcall *fn)())
{
...
long g2int_signal_handler_for_launch()
{
  ...
  if (g2int_signal_handler_for_launch_call_handle) {
    return_value = (*g2int_signal_handler_for_launch_call_handle)();
  } else {
 ...
 */
static long (__stdcall *g2int_network_event_handler_call_handle)(long, long, long);

typedef long (__stdcall *lisp_callable)(long,long,long);

lisp_callable fptr = NULL;

DLLEXPORT void g2int_network_event_handler_init_call_handle (long (__stdcall *fn)(long, long, long))
{
  fprintf(stderr, "Initialize call handle %d\n", fn);
...
long g2int_network_event_handler(long event_code, long event_socket_handle, long event_status_code)
{
  ...
  if (g2int_network_event_handler_call_handle) {
    // Add by Glority: directly return the valud of lisp call will fail,
    //  so we add one temporary variable. Now it can work.
    return_value = (*g2int_network_event_handler_call_handle)
                     (event_code, event_socket_handle, event_status_code);
  } else {
    fprintf(stderr, "g2int_network_event_handler_call_handle is still unbound\n");
    fprintf(stderr, "fptr: %d\n", fptr);
    return_value = -1L;
  }
  return return_value;
}
 ...
 */
static long (__stdcall *g2int_open_fd_callback_call_handle)(long);

DLLEXPORT void g2int_open_fd_callback_init_call_handle (long (__stdcall *fn)(long))
{
...
long g2int_open_fd_callback(long fd)
{
  ...
  if (g2int_open_fd_callback_call_handle) {
    return_value = (*g2int_open_fd_callback_call_handle)(fd);
  } else {
 ...
 */
static long (__stdcall *g2int_close_fd_callback_call_handle)(long);

DLLEXPORT void g2int_close_fd_callback_init_call_handle (long (__stdcall *fn)(long))
{
...
long g2int_close_fd_callback(long fd)
{
  ...
  if (g2int_close_fd_callback_call_handle) {
    return_value = (*g2int_close_fd_callback_call_handle)(fd);
  } else {
 ...
 */
static long (__stdcall *g2int_writing_fd_callback_call_handle)(long);

DLLEXPORT void g2int_writing_fd_callback_init_call_handle (long (__stdcall *fn)(long))
{
...
long g2int_writing_fd_callback(long fd)
{
  ...
  if (g2int_writing_fd_callback_call_handle) {
    return_value = (*g2int_writing_fd_callback_call_handle)(fd);
  } else {
 ...
 */
static long (__stdcall *g2int_not_writing_fd_callback_call_handle)(long);

DLLEXPORT void g2int_not_writing_fd_callback_init_call_handle (long (__stdcall *fn)(long))
{
...
long g2int_not_writing_fd_callback(long fd)
{
  ...
  if (g2int_not_writing_fd_callback_call_handle) {
    return_value = (*g2int_not_writing_fd_callback_call_handle)(fd);
  } else {
 ...
 */
static long (__stdcall *g2int_run_trace_function_call_handle)();

DLLEXPORT void g2int_run_trace_function_init_call_handle (long (__stdcall *fn)())
{
...
long g2int_run_trace_function()
{
  ...
  if (g2int_run_trace_function_call_handle) {
    return_value = (*g2int_run_trace_function_call_handle)();
  } else {
 ...
 */
static long (__stdcall *g2int_unix_command_callback_call_handle)();

DLLEXPORT void g2int_unix_command_callback_init_call_handle (long (__stdcall *fn)())
{
...
long g2int_unix_command_callback()
{
  ...
  if (g2int_unix_command_callback_call_handle) {
    return_value = (*g2int_unix_command_callback_call_handle)();
  } else {
 ...
 */
static long (__stdcall *nms_fetch_help_string_call_handle)(long, long);

DLLEXPORT void nms_fetch_help_string_init_call_handle (long (__stdcall *fn)(long, long))
{
...
long nms_fetch_help_string(long native_window, long item_id)
{
  ...
  if (nms_fetch_help_string_call_handle) {
    return_value = (*nms_fetch_help_string_call_handle)(native_window, item_id);
  } else {
 ...
 */
static long (__stdcall *nms_handle_if_flag_bit_call_handle)(long, long, long);

DLLEXPORT void nms_handle_if_flag_bit_init_call_handle (long (__stdcall *fn)(long, long, long))
{
...
long nms_handle_if_flag_bit(long hmenu_high, long hmenu_low, long flag)
{
  ...
  if (nms_handle_if_flag_bit_call_handle) {
    return_value = (*nms_handle_if_flag_bit_call_handle)(hmenu_high, hmenu_low, flag);
  } else {
 ...
 * NMS-ID-FOR-MENU
 */
static long (__stdcall *nms_id_for_menu_call_handle)(long, long);

DLLEXPORT void nms_id_for_menu_init_call_handle (long (__stdcall *fn)(long, long))
{
...
long nms_id_for_menu(long hmenu_high, long hmenu_low)
{
  ...
  if (nms_id_for_menu_call_handle) {
    return_value = (*nms_id_for_menu_call_handle)(hmenu_high, hmenu_low);
  } else {
