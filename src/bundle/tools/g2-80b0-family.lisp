...
(def-family G2-80b0
    :index 34			; per tools/datafiles/families/family-id.dat
    ...
    :guid "74ee5cc5-de60-4cd0-8012-c8da127d35b8" ; 8.0b0
    :compatible-previous-versions (("8.0a1" "162d35e4-346d-426a-b840-b3495f4c51a4")
...
(defvar g2-shortcut-folder-name "Gensym G2 8.0b0")
...
(def-component g2
  ...
  (g2-intelnt g2-unix
	      ...
	      g2-blank-ok
	      kbs fonts mill-demo startserver-bat-file)
...
(def-component activexlink
  :parts (axl axl-g2com-dll axl-demos)
  :platforms ("nt")
...
(def-component corbalink :parts (corbalink corbalink-10r3)
...
(def-component Oraclebridge :parts (Oraclebridge-6 gsi-Oracle-6-ok
				    Oraclebridge gsi-Oracle-ok run-g2-ora7-template-batfile run-g2-ora80-template-batfile run-g2-ora81-template-batfile run-g2-ora90-template-batfile run-g2-ora92-template-batfile gsi-Oracle-blank-ok)
	       :print-name "G2 Oracle Bridge"
	       :optional true)
(def-component Sybasebridge :parts (Sybasebridge-6 gsi-Sybase-6-ok
				    Sybasebridge gsi-Sybase-ok run-g2-sybase-template-batfile gsi-Sybase-blank-ok)
...
(def-shortcut g2-shortcut
    ...
    :target-name "g2.exe"
    :parameters "-netinfo -module-search-path kbs\\utils")
...
(def-registry-entry tw-registry-entry
    :path  "HKEY_LOCAL_MACHINE\\SOFTWARE\\Gensym\\Telewindows\\8.0 Beta Rev. 0" 
    :key-name "installDir"
    :value "%ROOTDIR%\\g2")
...
(def-part workspace-view
    :version :80b0
    :destination-directory "g2"
    :source-filename ("WSView.frm" "WSView.frx" "WorkspaceView.ocx" "WorkspaceViewDemo.vbp" "WorkspaceViewDemo.vbw" "registerWSV.bat" "unregisterWSV.bat" "workspace-views.kb" "wsvdemo.kb" "wsview.html")
    :platforms ("nt"))
...
(def-part g2i-core-modules
  ...
  :source-filename ("gens.kb" "gerr.kb" "grtl.kb"  "glf.kb" "gqs.kb" "gqsui.kb" "gqsviews.kb" "gqsdemo.kb" "resources-english.txt" "config.txt" "gevm.kb" "gdsm.kb")
...
(def-part corbalink
  ...
  :platforms ("sparcsol" "nt" "linux"))

(def-part corbalink-10r3
  :source corbalink
  :version :10r3
  :destination-directory corbalink
  :platforms ("hpux" "aix"))
...
(def-part axl
    :version :11b0
    :source axl
    :source-filename ("gsi.dll" "g2com.kb" "runme.bat" "regsvr32.exe")
...
(def-gsi-bridge-ok-parts (:obfuscate t :version 7 :non-platforms ("hpux" "linux"))
    oracle)

(def-gsi-bridge-ok-parts (:obfuscate t :platforms ("hpux" "linux"))
    (oracle-6 oracle))

(def-gsi-bridge-ok-parts (:obfuscate t :version 7 :non-platforms ("hpux" "linux"))
    sybase)

(def-gsi-bridge-ok-parts (:obfuscate t :platforms ("hpux"))
    (sybase-6 sybase))
...
(def-part Sybasebridge
    ...
    :non-platforms ("hpux" "linux")) ; "linux" does not exist for sybase

(def-part Sybasebridge-6
    :version :60r10
    :source g2sybase
    :destination-directory sybase
    :platforms ("hpux"))
...
(def-part Oraclebridge
    ...
    :non-platforms ("hpux" "linux"))

(def-part Oraclebridge-6
    :version 60r10
    :source g2oracle
    :destination-directory oracle
    :platforms ("hpux" "linux"))
