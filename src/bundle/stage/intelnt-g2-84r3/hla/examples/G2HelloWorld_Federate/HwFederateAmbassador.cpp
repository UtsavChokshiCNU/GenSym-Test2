...
throw (
  RTI::FederateInternalError)
{
   extern char	__syncPointName[128];
   extern long	__syncPointFlag;
   long i;
   char *sP = (char *) label;

   cerr << "FED_HW: announceSynchronizationPoint in FED. Setting flags." << endl;

   // This federate will always respond as it reached the sunchronization point.
   // Since we cannot call rtiAmb methods from here, we have to use and indiract way
   // We set a flag that will be detected in the main loop and inform the rti that we
   // reached the synchronization point.
   i = 0;
   while (sP[i] != 0 && i < 127) {

	__syncPointName[i] = sP[i];
	i++;
   }

   __syncPointName[127] = 0;
   __syncPointFlag = 1;
}
  ...
  RTI::FederateInternalError)
{
   ...
   if ( pCountry )
   {
      ...
      //-----------------------------------------------------------------
      pCountry->Update( theAttributes );
      pCountry->SetLastTime( RTIfedTime(0.0) );
   }
}
