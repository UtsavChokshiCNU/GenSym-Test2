...
*                                                                                    *
*     $Header: /media/gensym/home/versata1/cvs_repo_copyDec06/ab-repos/fnt/c/fscdefs.h,v 1.6 2004-04-19 16:35:36 mhd Exp $                                                                       *
*                                                                                    *
*     $Log: not supported by cvs2svn $
*     Revision 1.5  1998/04/08 21:33:14  jv
*     Project: Korean Fonts
*     Author: rh
*     Witness: jv
*
*     Change "long" to "int" wherever appropriate.  This will make no
*     difference on 32 bit platforms, but fixes problems on Alpha OSF.
*
*     Revision 1.4  1998/04/08 21:13:52  jv
*     Project: Korean Fonts
*
*     Need to mask the result of the previous SWAPW with 0xffff.  Sometimes
*     it works without the mask, but we can't count on it.
*
*     Revision 1.3  1998/04/08 18:41:23  jv
*     Project: Korean fonts
*     Witness: rh, mhd
*
*     In mapstrng.c, the macro for BinaryIteration needs to do a SWAPW
*     on the value at newP before comparing it to charCode.
*
*     In fscdefs.h, use the non-INTEL version of SWAPINC, somewhat modified,
*     all the time.  Don't cast the result of SWAPW to short.
*
*     Revision 1.2  1997/05/09 17:26:00  mhd
*     Witness: Shawn Flynn (Bitstream Developer and Paid Consultant)
*
*     This commit was suggested and authorized by Bitstream to their own source
*     code to fix problems experienced dealing with very large fonts (e.g.,
*     Asian fonts such Korean) on certain platforms.   A safer way to do
*     byte swapping,  when needed, that does not rely as much on specifics
*     of the machine architecture, and thereby avoids the access violations
*     we had been experiencing in evaluating various TrueType Korean fonts.
*
*     Revision 1.1  1995/11/13 22:32:00  mes
*     Project:  Bitstream Font Transition
*
*       Committing all Bitstream fonts files from the ext/c module.  This is the new
*     home for libfnt.a.
*
*     Revision 1.1  1995/10/03 00:35:54  jra
*     Adding ext files from 4.0 Rev. 1 as initial commit for AB.
*
*     Revision 1.1  1993/05/19 22:02:36  mhd
*     Added Bitstream fontrip (Font Raster Image Processor) c & h files.  Not
*     all are compiles -- some are demos, or test modules.
*
 * Revision 1.1  1993/05/04  20:13:40  mhd
 * The flat-directory version of the Bitstream 4-in-1 software.
 *
 * Revision 1.1.1.1  1993/04/15  20:14:37  mhd
...
#define SWAPW(a)		(((((unsigned char)((a) >> 8)) | ((unsigned char)(a) << 8))) & 0xffff)
...
#define SWAPWINC(a)     (++(a), ((((short)(((unsigned char *) (a - 1))[0])) << 8) + \
                                 (((unsigned char *) (a - 1))[1])))
