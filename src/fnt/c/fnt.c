 ...
 *	$Header: /media/gensym/home/versata1/cvs_repo_copyDec06/ab-repos/fnt/c/fnt.c,v 1.2 2005-05-23 22:36:08 jv Exp $
 *	$Log: not supported by cvs2svn $
 *	Revision 1.1  1995/11/13 22:31:33  mes
 *	Project:  Bitstream Font Transition
 *
 *	  Committing all Bitstream fonts files from the ext/c module.  This is the new
 *	home for libfnt.a.
 *
 *	Revision 1.1  1995/10/03 00:35:40  jra
 *	Adding ext files from 4.0 Rev. 1 as initial commit for AB.
 *
 *	Revision 1.3  1994/01/21 17:43:53  jra
 *	Project: Stabilization
 *
 *	Fixed fnt.c so that the function fnt_MSIRP matched its extern declaration,
 *	which included static.
 *
 * Revision 1.2  1993/12/30  20:31:21  mhd
 * Added static to the forward reference declarations to agree with the actual
 * definitions later in the file that are preceeded with static.  This will
 * hopefully silence the compiler on the aviion and motorola platforms, which
 * complained loudly about the disagreement.
 *
 * Also, removed the bogus extern declaration for fnt_SVTCA_0 entirely.
 *
 * Revision 1.1  1993/05/19  22:02:19  mhd
 * Added Bitstream fontrip (Font Raster Image Processor) c & h files.  Not
 * all are compiles -- some are demos, or test modules.
 *
 * Revision 1.1  1993/05/04  20:12:43  mhd
 * The flat-directory version of the Bitstream 4-in-1 software.
 *
 * Revision 1.1.1.1  1993/04/15  20:14:35  mhd
...
#ifdef RCSSTATUS
static char rcsid[] = "$Header: /media/gensym/home/versata1/cvs_repo_copyDec06/ab-repos/fnt/c/fnt.c,v 1.2 2005-05-23 22:36:08 jv Exp $";
#endif
...
static void fnt_IllegalInstruction PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_Normalize PROTO((F26Dot6 x, F26Dot6 y, VECTOR *v));
static void fnt_MovePoint PROTO((fnt_LocalGraphicStateType *gs,
				   fnt_ElementType *element,
				   ArrayIndex point,
				   F26Dot6 delta));
static void fnt_XMovePoint PROTO((fnt_LocalGraphicStateType *gs,
					fnt_ElementType *element,
					ArrayIndex point,
					F26Dot6 delta) );
static void fnt_YMovePoint PROTO((fnt_LocalGraphicStateType *gs,
					fnt_ElementType *element,
					ArrayIndex point,
					F26Dot6 delta) );
static F26Dot6 fnt_Project PROTO((fnt_LocalGraphicStateType *gs,
					F26Dot6 x,
					F26Dot6 y));
static F26Dot6 fnt_OldProject PROTO((fnt_LocalGraphicStateType *gs,
					   F26Dot6 x,
					   F26Dot6 y));
static F26Dot6 fnt_XProject PROTO((fnt_LocalGraphicStateType *gs, F26Dot6 x, F26Dot6 y));
static F26Dot6 fnt_YProject PROTO((fnt_LocalGraphicStateType *gs, F26Dot6 x, F26Dot6 y));
static Fixed fnt_GetCVTScale PROTO((fnt_LocalGraphicStateType *gs));
static F26Dot6 fnt_GetCVTEntryFast PROTO((fnt_LocalGraphicStateType *gs, ArrayIndex n));
static F26Dot6 fnt_GetCVTEntrySlow PROTO((fnt_LocalGraphicStateType *gs, ArrayIndex n));
static F26Dot6 fnt_GetSingleWidthFast PROTO((fnt_LocalGraphicStateType *gs));
static F26Dot6 fnt_GetSingleWidthSlow PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_ChangeCvt PROTO((fnt_LocalGraphicStateType *gs,
				   fnt_ElementType *element,
				   ArrayIndex number,
				   F26Dot6 delta));
static void fnt_InnerTraceExecute PROTO((fnt_LocalGraphicStateType *gs, uint8 *ptr, uint8 *eptr));
static void fnt_InnerExecute PROTO((fnt_LocalGraphicStateType *gs, uint8 *ptr, uint8 *eptr));
static void fnt_Check_PF_Proj PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_ComputeAndCheck_PF_Proj PROTO((fnt_LocalGraphicStateType *gs));
static Fract fnt_QuickDist PROTO((Fract dx, Fract dy));
static void fnt_SetRoundValues PROTO((fnt_LocalGraphicStateType *gs, int arg1, int normalRound));
static F26Dot6 fnt_CheckSingleWidth PROTO((F26Dot6 value, fnt_LocalGraphicStateType *gs));
static fnt_instrDef* fnt_FindIDef PROTO((fnt_LocalGraphicStateType* gs, uint8 opCode));
static void fnt_DeltaEngine PROTO((fnt_LocalGraphicStateType *gs,
					 FntMoveFunc doIt,
					 int16 base,
					 int16 shift));
static void fnt_DefaultJumpTable PROTO(( voidFunc* function ));

/* Actual instructions for the jump table */
static void fnt_SVTCA_0 PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SVTCA_1 PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SPVTCA PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SFVTCA PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SPVTL PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SDPVTL PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SFVTL PROTO(	(fnt_LocalGraphicStateType *gs));
static void fnt_WPV PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_WFV PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_RPV PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_RFV PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SFVTPV PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_ISECT PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SetLocalGraphicState PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SetElementPtr PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SetRoundState PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SROUND PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_S45ROUND PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_LMD PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_RAW PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_WLSB PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_LWTCI PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_LSWCI PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_LSW PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_DUP PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_POP PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_CLEAR PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SWAP PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_DEPTH PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_CINDEX PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_MINDEX PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_ROTATE PROTO(( fnt_LocalGraphicStateType* gs ));
static void fnt_MDAP PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_MIAP PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_IUP PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SHP PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SHC PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SHE PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SHPIX PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_IP PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_MSIRP PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_ALIGNRP PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_ALIGNPTS PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SANGW PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_FLIPPT PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_FLIPRGON PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_FLIPRGOFF PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SCANCTRL PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SCANTYPE PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_INSTCTRL PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_AA PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_NPUSHB PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_NPUSHW PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_WS PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_RS PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_WCVT PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_WCVTFOD PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_RCVT PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_RC PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_WC PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_MD PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_MPPEM PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_MPS PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_GETINFO PROTO((fnt_LocalGraphicStateType* gs));
static void fnt_FLIPON PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_FLIPOFF PROTO((fnt_LocalGraphicStateType *gs));
#ifndef NOT_ON_THE_MAC
#ifdef DEBUG
void fnt_DDT PROTO((int8 c, int32 n));
#endif
#endif
static void fnt_DEBUG PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SkipPushCrap PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_IF PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_ELSE PROTO(( fnt_LocalGraphicStateType* gs ));
static void fnt_EIF PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_JMPR PROTO(( fnt_LocalGraphicStateType* gs ));
static void fnt_JROT PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_JROF PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_BinaryOperand PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_UnaryOperand PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_ROUND PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_NROUND PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_PUSHB PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_PUSHW PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_MDRP PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_MIRP PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_CALL PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_FDEF PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_LOOPCALL PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_IDefPatch PROTO(( fnt_LocalGraphicStateType* gs ));
static void fnt_IDEF PROTO(( fnt_LocalGraphicStateType* gs ));
static void fnt_UTP PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SDB PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_SDS PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_DELTAP1 PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_DELTAP2 PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_DELTAP3 PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_DELTAC1 PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_DELTAC2 PROTO((fnt_LocalGraphicStateType *gs));
static void fnt_DELTAC3 PROTO((fnt_LocalGraphicStateType *gs));
...
/* extern void fnt_SVTCA_0();
 -- REMOVED -- it's defined (static) in this file
 (mhd@gensym, 12/30/93)
 */
  ...
  fnt_LocalGraphicStateType *gs;
{
	...
	if ( pfProj > -ONESIXTEENTHVECTOR && pfProj < ONESIXTEENTHVECTOR) {
		gs->pfProj = (VECTORTYPE) (pfProj < 0 ? -ONEVECTOR : ONEVECTOR);
		/* Prevent divide by small number */
	}
}
  ...
  register fnt_LocalGraphicStateType *gs;
{
	...
	if ( pfProj > -ONESIXTEENTHVECTOR && pfProj < ONESIXTEENTHVECTOR) {
		pfProj = (VECTORTYPE) (pfProj < 0 ? -ONEVECTOR : ONEVECTOR);
		/* Prevent divide by small number */
	}
	gs->pfProj = pfProj;
}
...
static void fnt_SHPIX (gs)
  register fnt_LocalGraphicStateType *gs;
{
    register F26Dot6 proj, dx = 0, dy = 0;
