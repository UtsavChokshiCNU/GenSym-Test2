...
*  Revision 1.1  1995/10/03 00:37:59  jra
*  Adding ext files from 4.0 Rev. 1 as initial commit for AB.
*
*  Revision 1.1  1993/05/19 22:04:55  mhd
*  Added Bitstream fontrip (Font Raster Image Processor) c & h files.  Not
*  all are compiles -- some are demos, or test modules.
*
 * Revision 1.1  1993/05/04  20:15:57  mhd
 * The flat-directory version of the Bitstream 4-in-1 software.
 *
 * Revision 1.1.1.1  1993/04/15  20:14:53  mhd
 * Initial checkin of Version 2.1 of 4-in-1 from Bitstream.  -jra & mpc
 *
 * Revision 2.19  93/03/15  14:00:28  roberte
 * Release
 * 
 * Revision 1.7  93/03/15  13:48:47  roberte
 * Added comments.
 * 
 * Revision 1.6  93/03/12  00:23:23  roberte
 * Added code to release/unload TrueType and PCL font memory.
 * 
 * Revision 1.5  93/03/11  13:49:25  weili
 * added fontfile name on the page header.
 * 
 * Revision 1.4  93/03/10  19:39:35  weili
 * added Log header,
 * added "if (set_width==0) in the beginning of
...
static	boolean	reset_spec(fix15 scan_lines);
...
static char proof_string[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqrstuvwxyz";
...
void hp_proof_prologue(void);
void hp_proof_epilog(void);
void hpwc(char *);
void layout (void);
...
/****************************************************************************
   FILE:       wfdemo.c
   This module demonstrates how to display or print waterfalls with 
   consecutivee lines of text of increasing point size.
*****************************************************************************/
int	main(argc,argv)
int argc;
char *argv[];
{
	...
	union REGS	regs;
   ufix16   start_offset = 2;
   ufix16   char_id;

	/* check for proper usage */
	if (argc < 2) 
   {
      printf("Usage:  wfdemo <fontfile> [/s] [/p]\n"); 
   ...
   {   
      do_proof = TRUE;
      pointsize = 6;
      scan_lines = (fix15) (300 / PT_PER_INCH * pointsize);
   }
   ...
   if (do_proof)
   {
      fix15    wf_end=27;
	   ...
	   {
		   case procType1:
	        sprintf(title_str, "Type1 %s Waterfall Sampler\n", pathname);
         	fprintf(stderr, "Processing Type1 fontfile %s ...\n", pathname);
   			break;
		   case procSpeedo:
	        sprintf(title_str, "Speedo %s Waterfall Sampler\n", pathname);
         	fprintf(stderr, "Processing Speedo fontfile %s ...\n", pathname);
   			break;
		   case procTrueType:
	        sprintf(title_str, "TrueType %s Waterfall Sampler\n", pathname);
         	fprintf(stderr, "Processing TrueType fontfile %s ...\n", pathname);
   			break;
		   case procPCL:
	        sprintf(title_str, "PCLeo %s Waterfall Sampler\n", pathname);
         	fprintf(stderr, "Processing PCLeo fontfile %s ...\n", pathname);
   			break;
   	}
      hp_proof_prologue();
      for ( ; pointsize <= wf_end; pointsize++)
      {
         scan_lines = 300 / PT_PER_INCH * pointsize;
         reset_spec(scan_lines);
         hp_proof_prologue_no_open();
   	   for (char_index = 0; char_index < 62; char_index++)   /* For each character in encoding */
         {
            hp_char_id = char_index;
   ...
   else /* output on the screen */
   {
      ...
      set_backgd();
       ...
       y_dev=scan_lines;
       while ( (460-y_dev) > scan_lines )
       {
            x_dev = 1; 
            char_index = 0;
            reset_spec(scan_lines);
            while ( char_index < 62 && x_dev < 640  )
            {
               char_id = proof_string[char_index];   
      		 	gBitmapWidth = SetWidthToPixelWidth(fi_get_char_width(&char_id));
               fi_make_char(&char_id);
               x_dev += gBitmapWidth + 2 ;
               char_index++;
            }
            y_dev += scan_lines;
            scan_lines++;
		...
		printf("\n Press any key to go back to text mode");
 		ch=getch();
		vga_CLS();
...
/****************************************************************************
  reset_spec()
      ...
      fi_set_specs().  Whenever the point size is changed, this function
      should be called before making characters.
  RETURNS:
      TRUE if successful
      FALSE if failed
*****************************************************************************/
...
*/
{
   ...
   if (do_proof == TRUE)
   {
      ...
      hpfcdir.size = 14;
      hpfcdir.class = 1;
      hpfcdir.orientation = 0;  /* portrait */
      hpfcdir.reserved = 0;
      hpfcdir.xoff = offhor;
...
swap_cdir(&hpfcdir);
   ...
   else
   {
      ...
      xpix = (x_dev*65536 + xorg + 32768L) >> 16;
      ypix = (y_dev*65536 + 32768L) >> 16;
      if (raswid > VIDEO_MAX)
          raswid = VIDEO_MAX;
...
*/
{
   ...
   if (xpix + xbit1 + length > 640)
      length = 640 - xpix -xbit1;

	vga_hline(xpix+xbit1,ypix-rashgt-offver+y,length);
...
/****************************************************************************
  hp_proof_prologue()
      This function opens the output file for printer proof, setupt the 
      orientation for the page(i.e. landscape mode), and send the title
      string to the file.
*****************************************************************************/
void hp_proof_prologue(void)
{
   ...
   hpwc("\033E");
   hpwc("\033&l1O");  /* set the logical page to be landscape mode */
   sprintf(command,"\033*p+100Y\033*p0X\33(0U\33(s1p16v0s3b4101T%s", title_str);
   hpwc(command);
   hpwc("\033*p350Y");		
   hpwc("\033*p0X");	
...
/****************************************************************************
  hp_proof_epilog_no_close()
      This function select the soft font to use and sends the characters
...
/****************************************************************************
  hp_proof_epilog()
      This function sends footer to the file, does the clean up, and close
      the output file.
*****************************************************************************/
void hp_proof_epilog(void)
{
   ...
   sprintf(command,"\033*p2700X\033*p2100Y\33(0U\33(s1p12v0s0b4101T%s", "Bitstream, Inc.");
   hpwc(command);
...
void layout ()
{
   ...
   sprintf(command,"\33(0U\33(s1p6v0s0b4101T%d", pointsize);
   hpwc(command);
   hpwc("\033*p120X");
   ...
   hor_index =  48/(PT_PER_INCH/(pointsize));
   sprintf(command, "\33&l%.4fC", hor_index);
  	hpwc(command);
...
#ifdef TRIED_AND_FAILED
	/* 1st tried and failed: */
    p->pitch = (int)(space_width * 1/65536 * 	PROOF_CHAR_HEIGHT * 1/PT_PER_INCH * 300 * 4);

	/* another tried and failed: */
	p->pitch = (ufix32)((float)space_width / (float)65536 * (float)PROOF_CHAR_HEIGHT / (float)PT_PER_INCH * (float)300 * (float)4);
#else
...
//	OUTINDEX(0, 0x0b);  /* set light blue*/
