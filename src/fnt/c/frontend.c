 ...
 * $Header: /media/gensym/home/versata1/cvs_repo_copyDec06/ab-repos/fnt/c/frontend.c,v 1.2 2005-05-23 22:36:09 jv Exp $
 *
 * $Log: not supported by cvs2svn $
 * Revision 1.1  1995/11/13 22:31:59  mes
 * Project:  Bitstream Font Transition
 *
 *   Committing all Bitstream fonts files from the ext/c module.  This is the new
 * home for libfnt.a.
 *
 * Revision 1.1  1995/10/03 00:35:53  jra
 * Adding ext files from 4.0 Rev. 1 as initial commit for AB.
 *
 * Revision 1.6  1994/11/15 23:26:47  mpc
 * Project: NT Sentinel Keys
 *
 * The Sentinel SuperPro library had a name conflict with a libfnt.lib function,
 * sp_reset(). Rather than a warning at link time, MS Linker simply did a 50-50
 * pick  between the two instances. Since we have source control over the font
 * library, changed function name sp_reset() to spd_reset().
 *
 * Revision 1.5  1993/09/21  13:57:37  mhd
 * Restored bsBsearch renaming, which had accidentally been removed by my
 * last change.
 *
 * Revision 1.3  1993/08/27  20:32:51  jed
 * Fixing mult definition of bsearch() bug
 *
 * Revision 1.2  1993/08/19  04:52:12  mhd
 ...
 * minor bug fixes.
 *
 * Revision 1.1  1993/05/04  20:13:38  mhd
 * The flat-directory version of the Bitstream 4-in-1 software.
 *
 * Revision 1.2  1993/04/22  19:24:25  mhd
 * PFB sv_init_out, sv_begin_char, sv_end_char;
 * New type defined (PFB) -- and these variable declarations
 * out of PFV (void) to PFB (boolean) to comply with SGI's
 * strict ansi interpretation.
 *
 * The correctness was surmised by examining the context: these
 * variables were just used to temporarily store old values in
 * the structures.  Then the structures were set with booleans
 * rather than voids in the code that followed.  After that code,
 * the structures were restored from these variables.
 *
 * Revision 1.1.1.1  1993/04/15  20:14:48  mhd
...
boolean return_value = FALSE; /* assume the worst */
	if (sp_globals.gMustTranslate)
		...
		if (0)
		  /* ( (*intPtr = sp_globals.gCurrentSymbolSet[*iPtr]) == UNKNOWN)
		     -- the above was replaced with FALSE to gag the following warning
		     from GCC:
		       frontend.c: In function `DoTranslate':
		       frontend.c:628: warning: comparison is always 0 due to limited range of data type
		     As the warning points out, the original is always 0 (FALSE) anyway.
		     -- mhd@gensym, 9/11/93
		     */
			return(return_value); /* already FALSE */
...
boolean success = FALSE;
	outValue = 0;
	success = bsBSearch (PARAMS2  (fix15 STACKFAR*)&outValue, BCIDCompare,
