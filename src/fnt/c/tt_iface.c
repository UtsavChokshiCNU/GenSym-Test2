 ...
 *
*     $Header: /media/gensym/home/versata1/cvs_repo_copyDec06/ab-repos/fnt/c/tt_iface.c,v 1.4 2005-05-31 02:42:44 jv Exp $
*     $Log: not supported by cvs2svn $
*     Revision 1.3  2005/05/23 22:36:11  jv
*     Project: Warnings
*     Witness: none
*     Authorization: jv
*
*     Files modified: fnt/c/cmt.c fnt/c/do_char.c fnt/c/fnt.c fnt/c/fontrip.c
*        fnt/c/frontend.c fnt/c/fsglue.c fnt/c/mapstrng.c fnt/c/msl.c fnt/c/newscan.c
*        fnt/c/out_bl2d.c fnt/c/out_blk.c fnt/c/out_scrn.c fnt/c/out_wht.c
*        fnt/c/reset.c fnt/c/set_keys.c fnt/c/set_spcs.c fnt/c/set_trns.c
*        fnt/c/sfntd.c fnt/c/tt_iface.c fnt/c/tt_specs.c
*
*     Description of changes: make some edits to eliminate warnings when compiling the
*        fnt library on various platforms.
*
*        This fell into several categories.  A lot of the changes are just adding
*        explicit casts.  Add parentheses to clarify precedence in certain cases, to
*        quiet the Microsoft compiler.  Make sure functions declared to return values
*        actually return values.  Remove unused variables; if a variable is only used
*        in a certain #ifdef case, then only define it in that #ifdef case.  Include
*        <math.h> in fsglue.c.  A couple of whitespace changes.
*
*     Difficulty of change: trivial
*     Number of edit sites: 63
*     Number of lines changed: 64
*     When and how bug was introduced: original implementation
*
*     Fanin and fanout of change: eliminates some warnings, mostly on Microsoft.
*
*     How this has been tested: did a clean build of libfnt on all platforms.  There
*        are still warnings from fixed_pt.c on Windows, but that is now all that is
*        left.  Tested G2 on rs6000 and on Solaris, in Lisp.
*
*     Revision 1.2  2003/12/05 20:20:48  mhd
*     Project: Clean up some warnings in fontrip.c
*     Witness: jv
*     Authorization: mhd
*     Files modified and/or added: ext/c/fontrip.c ext/c/tt_iface.c
*     Description of changes: get rid of all three c-compiler warnings on sparcsol.
*
*       Add #include stdlib.h in both files to get rid of these:
*         fontrip.c:462: warning: implicit declaration of function `malloc'
*         fontrip.c:2353: warning: implicit declaration of function `free'
*         tt_iface.c:457: warning: implicit declaration of function `free'
*
*       Add prototype declaration in fontrip.c for text_x_to_text_width() to get rid
*       of this:
*
*         fontrip.c:864: warning: implicit declaration of function `text_x_to_text_width'
*
*       These were the only warnings currently in compiling the module fnt on
*       sparcsol.  (A few more are present on other platforms, at least on Windows
*       and Tru64 Unix, but I'll defer fixing those until a later time.)
*     Difficulty of change: very easy
*     Number of edit sites: 3
*     Number of lines changed: 3 (new lines)
*     Fanin and fanout of change: the code has to be correct and digestible on all the
*       compilers, but it should be, and if so, there is no fanin or fanout. It's just
*       declaring stuff the compiler figures out itself but wants to warn you about.
*     How tested: compiled on windows, sparcsol, and Compaq/HP Tru64. Compiled the fnt
*       module successfully on all three platforms. Ran resulting G2 on windows and
*       sparcsol.  There were no new warnings on Windows or Tru64 (a few old ones
*       remain), and no warnings at all on sparcsol.
*
*     Revision 1.1  1995/11/13 22:32:55  mes
*     Project:  Bitstream Font Transition
*
*       Committing all Bitstream fonts files from the ext/c module.  This is the new
*     home for libfnt.a.
*
*     Revision 1.1  1995/10/03 00:37:51  jra
*     Adding ext files from 4.0 Rev. 1 as initial commit for AB.
*
*     Revision 1.1  1993/05/19 22:04:42  mhd
*     Added Bitstream fontrip (Font Raster Image Processor) c & h files.  Not
*     all are compiles -- some are demos, or test modules.
*
 * Revision 1.1  1993/05/04  20:15:42  mhd
 * The flat-directory version of the Bitstream 4-in-1 software.
 *
 * Revision 1.2  1993/04/19  15:29:26  mpc
 * changed func def of tt_error() from void to static void, per bitstream.
 *
...
#ifdef RCSSTATUS
static char rcsid[] = "$Header: /media/gensym/home/versata1/cvs_repo_copyDec06/ab-repos/fnt/c/tt_iface.c,v 1.4 2005-05-31 02:42:44 jv Exp $"
#endif
...
if ((sp_globals.processor.truetype.iPtr->memoryBases[0] =
	(char *)malloc(sp_globals.processor.truetype.oPtr->memorySizes[0])) == NULL)
    goto malerr1;
if ((sp_globals.processor.truetype.iPtr->memoryBases[1] =  
	(char *)malloc(sp_globals.processor.truetype.oPtr->memorySizes[1])) == NULL)
    goto malerr1;
if ((sp_globals.processor.truetype.iPtr->memoryBases[2] =  
	(char *)malloc(sp_globals.processor.truetype.oPtr->memorySizes[2])) == NULL)
    goto malerr1;
...
if ((sp_globals.processor.truetype.iPtr->memoryBases[3] =  
	(char *)malloc(sp_globals.processor.truetype.oPtr->memorySizes[3])) == NULL)
    goto malerr2;
if ((sp_globals.processor.truetype.iPtr->memoryBases[4] =  
	(char *)malloc(sp_globals.processor.truetype.oPtr->memorySizes[4])) == NULL)
    goto malerr2;
...
FUNCTION static void tt_error(PARAMS2 error)
GDECL
int32 error;
{
...
fix31    round;
boolean  outside = FALSE;  /* value never changes -jv, 5/18/05 */
	...
	while (ptype != END_CHAR)
	    ...
	    switch(ptype)
	        ...
	        case MOVE:
	            ...
	            fn_begin_contour(Pmove, outside);
#ifdef FNDEBUG
...
if (dist2 != 0)
    {
    dist = (F26Dot6) floor((double)area2 * 64.0 / (double)dist2 + 0.5);
    /* F26Dot6 conversion -- dist is in  64ths */
...
if (depth == 0)
    ...
    if (sp_globals.processor.truetype.abshift >= 0)
        {
        Pline.x = (fix15) (midx << sp_globals.processor.truetype.abshift);
        Pline.y = (fix15) (midy << sp_globals.processor.truetype.abshift);
        fn_line(Pline);
#ifdef FNDEBUG
    printf("fn_line: %d %d\n", Pline.x, Pline.y);
#endif
        Pline.x = (fix15) (Cx << sp_globals.processor.truetype.abshift);
        Pline.y = (fix15) (Cy << sp_globals.processor.truetype.abshift);
        fn_line(Pline);
#ifdef FNDEBUG
    printf("fn_line: %d %d\n", Pline.x, Pline.y);
#endif
