...
Object Current_attribute_description_class = UNBOUND;
    ...
    Object class_name, slot_desc_alist;
{
    ...
    PUSH_SPECIAL(Current_attribute_description_class,current_attribute_description_class,
	    0);
...
Object all_virtual_attributes_for_class varargs_1(int, n)
{
  ...
  next_loop_1:
    ...
    if (valid_attribute) {
	ab_loopvar__4 = gensym_cons_1(valid_attribute,Nil);
	if (ab_loopvar__3)
    ...
    Object user_mode, expose_internal_classes_p;
{
    ...
    PUSH_SPECIAL(Current_attribute_description_class,current_attribute_description_class,
	  ...
	  returned_structure = gensym_cons_1(gensymed_symbol,Qab_structure);
	  if (system_defined_p) {
		  ...
		  if (EQ(convert_type_or_unit_of_measure_to_type(M_CAR(M_CDR(value))),
			  ...
			  evaluation_value_qm = 
				  gensym_cons_1(managed_value,Qtruth_value);
		  }
		  else
    ...
    Object type_specification;
{
	...
	if (EQ(type_or_operator,Qab_or) || EQ(type_or_operator,Qand)) {
	  ...
	  next_loop:
	    ...
	    ab_loop_list_ = M_CDR(ab_loop_list_);
	    ab_loopvar__1 = 
		    gensym_cons_1(generate_type_description_of_attribute(operand),
		    Nil);
	    if (ab_loopvar_)
	  ...
	  end_loop:
	    ...
	    if (FIXNUM_LE(ISVREF(gensymed_symbol,(SI_Long)1L),
		    Maximum_in_place_array_size)) {
		item_or_value = 
			allocate_evaluation_sequence(copy_list_using_gensym_conses_1(rest_of_spec));
		if (SIMPLE_VECTOR_P(item_or_value) && 
			IFIX(SIMPLE_ARRAY_ANY_1_LENGTH(item_or_value)) > 
			(SI_Long)2L &&  !EQ(ISVREF(item_or_value,
			(SI_Long)1L),Qavailable_frame_vector)) {
	    ...
	    minimum_contents_qm = details_qm ? CADDR(type_specification) : 
		    Qnil;
	    temp_2 = gensym_list_2(Qtype_or_operator,Qab_structure);
	    if (slots_qm) {
		slot_spec = Nil;
	    ...
	    temp_3 = gensym_list_2(Qtype_or_operator,Qsequence);
	    temp_2 = elt_type_qm ? gensym_list_2(Qelement_type,
	    ...
	    return allocate_evaluation_structure(nconc2(temp_3,
		    ...
		    gensym_list_2(Qminimum_elements,low_bound_qm) : Nil,
		    high_bound_qm ? gensym_list_2(Qmaximum_elements,
		    high_bound_qm) : Nil))));
	}
    ...
    Object boolean_combo;
{
	...
	ab_loop_list_ = M_CDR(ab_loop_list_);
	ab_loopvar__2 = 
		gensym_cons_1(generate_structure_required_attributes_sequences(elt_1),
		Nil);
	if (ab_loopvar__1)
	    M_CDR(ab_loopvar__1) = ab_loopvar__2;
	else
    ...
    Object frame, specific_attributes;
{
  ...
  next_loop:
	...
	ab_loopvar__1 = gensym_cons_1(name,Nil);
	if (ab_loopvar_)
	    M_CDR(ab_loopvar_) = ab_loopvar__1;
	else
	    attribute_names_and_values = ab_loopvar__1;
	ab_loopvar_ = ab_loopvar__1;
	ab_loopvar__1 = gensym_cons_1(attribute_value_qm,Nil);
	if (ab_loopvar_)
	    M_CDR(ab_loopvar_) = ab_loopvar__1;
	else
	    attribute_names_and_values = ab_loopvar__1;
    ...
    Object frame, specific_attributes;
{
  ...
  next_loop:
	...
	if (ab_loopvar_)
	    ...
	    attribute_names_and_values = ab_loopvar__1;
	ab_loopvar_ = ab_loopvar__1;
	ab_loopvar__1 = gensym_cons_1(attribute_text_value_qm,Nil);
	if (ab_loopvar_)
	    M_CDR(ab_loopvar_) = ab_loopvar__1;
	else
	    attribute_names_and_values = ab_loopvar__1;
    ...
    Object frame, denoted_attributes_sequence;
{
  ...
  next_loop:
    ...
    ab_loopvar__1 = gensym_cons_1(serve_text_of_attribute(frame,name,
	    class_qualifier_qm),Nil);
    if (ab_loopvar_)
	M_CDR(ab_loopvar_) = ab_loopvar__1;
    else
