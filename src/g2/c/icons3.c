    ...
    Object translator, new_state_information;
{
    ...
    svref_new_value = gensym_cons_1(new_state_information,
    ...
    Object translator, new_state_information;
{
	...
	goto next_loop;
      end_loop:
	temp = gensym_cons_1(new_state_information,Nil);
    ...
    Object translator;
{
	...
	reclaim_gensym_cons_1(state_information_qm);
...
{
    Object drawing_elements, icon_min_x, icon_min_y, icon_max_x, icon_max_y;
    Object result;
...
Object Distance_to_snap_points_together_in_view_pad = UNBOUND;
	...
	switch (INTEGER_TO_CHAR(temp)) {
	  ...
	  case 4:
		...
		return push_icon_editor_translator_state_information(translator,
			gensym_cons_1(x_in_view_pad,y_in_view_pad));
	    }
	    else
		return VALUES_1(Nil);
	  case 5:
		...
		if (state_information_qm) {
		    ...
		    reclaim_gensym_tree_1(state_information_qm);
		    handle_icon_editor_visual_event(icon_editor,
	  ...
	  case 6:
		...
		if (state_information_qm) {
		    ...
		    handle_icon_editor_visual_event(icon_editor,
			    FIX((SI_Long)15L),
			    copy_list_using_gensym_conses_1(FIRST(state_information_qm)),
			    gensym_cons_1(x_in_view_pad,y_in_view_pad));
		    handle_icon_editor_visual_event(icon_editor,
			    FIX((SI_Long)14L),x_in_view_pad,y_in_view_pad);
		    handle_icon_editor_visual_event(icon_editor,
	  ...
	  case 17:
		...
		if ( !TRUEP(state_information_qm)) {
		    push_icon_editor_translator_state_information(translator,
			    gensym_cons_1(FIXNUM_MINUS(x_in_workspace,
			    ISVREF(view_pad,(SI_Long)5L)),
			    FIXNUM_MINUS(y_in_workspace,ISVREF(view_pad,
			    (SI_Long)6L))));
		    push_icon_editor_translator_state_information(translator,
			    gensym_cons_1(x_in_workspace,y_in_workspace));
		}
	...
	if ( !(FIXNUM_LE(FIX((SI_Long)-128L),temp) && FIXNUM_LE(temp,
		...
		if ( !(FIXNUM_LE(FIX((SI_Long)-128L),temp) && 
			...
			if (state_information_qm) {
				...
				handle_icon_editor_visual_event(icon_editor,
					FIX((SI_Long)15L),
					gensym_cons_1(x0_in_view_pad,
					y0_in_view_pad),
					gensym_cons_1(x1_in_view_pad,
					y1_in_view_pad));
			...
			if (SECOND(state_information_qm)) {
			    ...
			    return handle_icon_editor_visual_event(icon_editor,
				    FIX((SI_Long)18L),
				    gensym_list_2(gensym_cons_1(x0_in_view_pad,
				    y0_in_view_pad),
				    gensym_cons_1(x1_in_view_pad,
				    y1_in_view_pad)),
				    gensym_cons_1(x2_in_view_pad,
				    y2_in_view_pad));
			}
			else
			    return VALUES_1(Nil);
