...
Object make_native_window varargs_1(int, n)
{
	...
	temp_1 = getfq_function_no_default(INLINE_SYMBOL_PLIST(system_1),
...
Object System_window = UNBOUND;

Object Current_process_handle = UNBOUND;

Object Current_system_version = UNBOUND;

Object Send_messages_to_logbook_p = UNBOUND;

Object Send_messages_to_console_p = UNBOUND;

Object Send_logbook_messages_to_console_p = UNBOUND;

Object Debug_frame_reference_balance_qm = UNBOUND;

Object Trace_system_calls_p = UNBOUND;

Object Echo_dialog_spec_p = UNBOUND;
...
Object launch_system_process(system_1)
    Object system_1;
{
    Object current_system_name, timeout, temp;
...
Object Generic_listener_initialized_qm = UNBOUND;

Object G2_stream_all_open_streams = UNBOUND;
...
{
    Object shutdown_catch_tag, inner_abort_level_tag, package, read_base;
    Object read_default_float_format, print_base, print_array, print_case;
    Object terminal_io, standard_input, error_output, debug_io, system_window;
    Object current_system_name, current_process_handle, current_system_version;
    Object within_managed_object_scope, gensymed_symbol;
    Object g2_stream_all_open_streams, copy_of_g2_stream_all_open_streams;
    ...
    PUSH_SPECIAL(Shutdown_catch_tag,shutdown_catch_tag,17);
      if (PUSH_CATCH(Shutdown_catch_tag,3)) {
	next_loop:
	  if (PUSH_CATCH(Qabort_level_0,2)) {
	      inner_abort_level_tag = Qabort_level_0;
	      PUSH_SPECIAL(Inner_abort_level_tag,inner_abort_level_tag,16);
		package = find_package(string_constant_2);
		PUSH_SPECIAL(Package,package,15);
		  read_base = FIX((SI_Long)10L);
		  PUSH_SPECIAL(Read_base,read_base,14);
		    read_default_float_format = Qdouble_float;
		    PUSH_SPECIAL(Read_default_float_format,read_default_float_format,
			    13);
		      print_base = FIX((SI_Long)10L);
		      PUSH_SPECIAL(Print_base,print_base,12);
			print_array = Nil;
			PUSH_SPECIAL(Print_array,print_array,11);
			  print_case = Kupcase;
			  PUSH_SPECIAL(Print_case,print_case,10);
			    terminal_io = Terminal_io;
			    PUSH_SPECIAL(Terminal_io,terminal_io,9);
			      standard_input = Terminal_io;
			      PUSH_SPECIAL(Standard_input,standard_input,8);
				error_output = Terminal_io;
				PUSH_SPECIAL(Error_output,error_output,7);
				  debug_io = Terminal_io;
				  PUSH_SPECIAL(Debug_io,debug_io,6);
				    system_window = Nil;
				    PUSH_SPECIAL(System_window,system_window,
					    5);
				      current_system_name = system_name;
				      PUSH_SPECIAL(Current_system_name,current_system_name,
					      4);
					current_process_handle = 
						make_symbol(string_constant_3);
					PUSH_SPECIAL(Current_process_handle,current_process_handle,
						3);
					  current_system_version = 
						  get(system_name,
						  Qsystem_version,_);
					  PUSH_SPECIAL(Current_system_version,current_system_version,
						  2);
					    if ( !EQ(Current_system_name,Qgsi))
						register_all_signal_handlers_for_launch();
					    within_managed_object_scope = T;
					    PUSH_SPECIAL(Within_managed_object_scope,within_managed_object_scope,
						    1);
						  ...
						  g2_stream_all_open_streams 
							  = Nil;
						  PUSH_SPECIAL(G2_stream_all_open_streams,g2_stream_all_open_streams,
							  0);
						    if (PUSH_UNWIND_PROTECT(0)) 
								{
							System_window = 
								initial_native_window_qm 
								? 
