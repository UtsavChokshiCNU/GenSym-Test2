...
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
    ...
    rect.DeflateRect(GetMargin(), 0); // Gensym change: put back code "changed by Yogurt" -fmw, 3/21/07
    ...
    if (pGrid->GetImageList() && GetImage() >= 0)
    {
        ...
        if (pGrid->GetImageList()->GetImageInfo(GetImage(), &Info)) {
		  ...
		  // Clip the image to space available, rather than simply giving up if
		  // there isn't enough room. Gensym change. -fmw, 9/21/06

		  int nImageWidth = Info.rcImage.right - Info.rcImage.left;            
		  int nImageHeight = Info.rcImage.bottom - Info.rcImage.top;            
		  int cx = min(nImageWidth, rect.right - rect.left);
		  int cy = min(nImageHeight, rect.bottom - rect.top);
		  pGrid->GetImageList()->DrawIndirect(pDC, GetImage(), rect.TopLeft(), CSize(cx,cy), CPoint(0,0));

            //rect.left += nImageWidth+GetMargin();
        }
    }

    // Draw sort arrow	TODO: Abstract this into its own method.
    ...
    GetTextRect(rect);			// FIXME: This is calling GetImageInfo() again.
    rect.right++;    
    rect.bottom++;
...
CSize CGridCellBase::GetTextExtent(LPCTSTR szText, CDC* pDC /*= NULL*/)
{
	...
	// Gensym: There's a bug here since the margin is derived from the font char
	// width, but it's being used as a delta-y, too. In any case, this can cause
	// new rows to have a different height from initial rows. A bug to be fixed
	// later. Currently the grid looks OK for the default font, Tahoma. -fmw,
	// 3/23/07
