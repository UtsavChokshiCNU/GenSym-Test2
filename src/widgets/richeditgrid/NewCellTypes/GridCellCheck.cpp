...
BOOL CGridCellCheck::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
    ...
    pDC->DrawFrameControl(GetCheckPlacement(), DFC_BUTTON, 
						  ...
						  (GetGrid()->IsCellEditable(nRow, nCol) ? 0 : DFCS_INACTIVE));
    // }
#endif
    return bResult;
}
...
// Gensym addition
extern void g2pvt_grid_view_check_box_clicked (CGridCellCheck *check_box,
...
void CGridCellCheck::OnClick(CPoint PointCellRelative)
{
	...
	if (GetCheckPlacement().PtInRect(PointCellRelative))
	{
		...
		// Gensym addition
		g2pvt_grid_view_check_box_clicked(this, PointCellRelative);
	}
}

// Gensym addition. We're not readonly, but edit-as-text does nothing.
BOOL CGridCellCheck::Edit(int nRow, int nCol, CRect rect, CPoint p, UINT nID, UINT nChar)
{
  return FALSE;								 // Ignored anyways.
}
