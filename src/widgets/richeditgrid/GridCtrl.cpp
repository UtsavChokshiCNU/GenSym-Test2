...
extern "C" void g2pvt_trace(char *fmt, ...); // Gensym addition
#define PRINTF g2pvt_trace		             // Gensym addition
...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
    ...
    m_AllowSelectRowInFixedCol = false; // GENSYMCID-1045
    ...
    m_nFreezedRows = 0; // GENSYMCID-1045
    m_nFreezedCols = 0; // GENSYMCID-1045

    m_bExcludeFreezedRowsFromSelection = FALSE; // GENSYMCID-1045
    m_bExcludeFreezedColsFromSelection = FALSE; // GENSYMCID-1045
    ...
    InvalidateFindRowCache();	// Gensym addition
    // GENSYMCID-1132: Grid-view sorting by first-column
    SetNumericHeader(FALSE);	// Gensym addition
...
#endif
    m_bShowHorzNonGridArea = TRUE; // GENSYMCID-1045
    m_nScrollX = 0;                // GENSYMCID-1045
}
...
BOOL CGridCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle)
{
    ...
    TRY
    {
        ...
        m_arRowTags.SetSize(m_nRows);		// Gensym Addition 
    }
    ...
    for (i = 0; i < m_nRows; i++)
    {
        ...
        m_arRowTags[i] = GENSYM_INVALID_ROW_TAG; // Gensym addition
    }
    for (i = 0; i < m_nCols; i++)
    ...
    else if (nChar == VK_RIGHT || (nChar == VK_TAB && !IsSHIFTpressed()) )
    {
        ...
        if (nChar == VK_TAB)
        {
            ...
            if (next.col == (GetColumnCount()) && next.row < (GetRowCount() - 1))
            {
                ...
                // Place focus on first non-fixed column
...
void CGridCtrl::OnDraw(CDC* pDC)
{
    ...
    pen.DeleteObject();
    pen.CreatePen(PS_SOLID, 3, RGB(0xc8, 0xc8, 0xc8));
    pDC->SelectObject(&pen);
...
#ifdef _DEBUG_DRAWING_PERFORMANCE
...
COleDataSource* CGridCtrl::CopyTextFromGrid()
{
    ...
    // Write Unicode chars to shared file
    ...
    for (int row = Selection.GetMinRow(); row <= Selection.GetMaxRow(); row++)
    {
        ...
        sf.Write(T2W(str.GetBuffer(0)), sizeof(wchar_t) * str.GetLength()); // Gensym: T2W
        str.ReleaseBuffer();
    }
    
    wchar_t c = L'\0';			     // Gensym
    sf.Write(&c, sizeof(wchar_t));	 // Gensym
    ...
    pSource->CacheGlobalData(CF_UNICODETEXT, hMem);	// Gensym change: use unicode
...
{
  // Various Gensym changes in here to use Unicode clipboard format. -fmw,
  // 9/22/06
    if (!IsValid(cell) || !IsCellEditable(cell) || !pDataObject->IsDataAvailable(CF_UNICODETEXT))
        return FALSE;

    // Get the text from the COleDataObject
    HGLOBAL hmem = pDataObject->GetGlobalData(CF_UNICODETEXT);
    int nbytes = ::GlobalSize(hmem);
    CMemFile sf((BYTE*) ::GlobalLock(hmem), nbytes);

    LPWSTR szBuffer = (LPWSTR)new BYTE[nbytes];
    if (!szBuffer)
        return FALSE;

    sf.Read(szBuffer, nbytes);
    ::GlobalUnlock(hmem);

    // Now store in generic TCHAR form so we no longer have to deal with
    // ANSI/UNICODE problems
    CString strText = W2T(szBuffer); // Gensym change: W text to T text -fmw, 9/22/06
    delete[] szBuffer;
...
DROPEFFECT CGridCtrl::OnDragOver(COleDataObject* pDataObject, DWORD dwKeyState,
                                 CPoint point)
{
  // Gensym changes: use Unicode clipboard format. -fmw, 9/22/06
    // Any text data available for us?
    if (!m_bAllowDragAndDrop || !IsEditable() || !pDataObject->IsDataAvailable(CF_UNICODETEXT))
...
{
    // Any text data available for us?
    if (!m_bAllowDragAndDrop || !pDataObject->IsDataAvailable(CF_UNICODETEXT))
        return DROPEFFECT_NONE;
...
void CGridCtrl::ResetSelectedRange()
{
    ...
    SetFocusCell(-1,-1);
    SendMessageToParent(m_idCurrentCell.row, m_idCurrentCell.col, GVN_SELCHANGED); // Gensym addition
}
...
void CGridCtrl::ResetScrollBars()
{
    ...
    */
    ...
    // GENSYMCID-1045: Feature to freeze the column/row in custom-dialog grid-view 
    // when the freezed columns out of view,the vertical scrollbar will never be visible,
    // we comment following line to fix this bug temparorily.
...
BOOL CGridCtrl::SetRowCount(int nRows)
{
    ...
    TRY
    {
        ...
        m_arRowTags.SetSize(nRows);     // Gensym Addition
        if (GetVirtualMode())
        {
            m_nRows = nRows;
            if (addedRows > 0)
            {
                ...
                for (int row = startRow; row < nRows; row++)
                {
                    ...
                    m_arRowTags[row] = GENSYM_INVALID_ROW_TAG; // Gensym addition
                }
            }
        }
        else
...
int CGridCtrl::InsertRow(LPCTSTR strHeading, int nRow /* = -1 */)
{
    ...
    TRY
    {
        ...
        if (nRow < 0)
        {
            ...
            m_arRowTags.Add(GENSYM_INVALID_ROW_TAG); // Gensym addition
            if (!GetVirtualMode())
        ...
        else
        {
            ...
            m_arRowTags.InsertAt(nRow, GENSYM_INVALID_ROW_TAG); // Gensym addition
            if (!GetVirtualMode())
	...
	InvalidateFindRowCache();	// Gensym addidion
...
BOOL CGridCtrl::DeleteRow(int nRow)
{
    ...
    m_arRowTags.RemoveAt(nRow);				   // Gensym Addition
...
BOOL CGridCtrl::DeleteAllItems()
{
    ...
    m_arRowTags.RemoveAll();				   // Gensym Addition
...
// Gensym Addition
extern BOOL g2pvt_use_numeric_sort_on_gv_col (CGridCtrl *grid, int nCol);
...
BOOL CGridCtrl::SortItems(int nCol, BOOL bAscending, LPARAM data /* = 0 */)
{
	...
	if (m_pfnCompare == NULL)
	  // Gensym Addition
	  if (g2pvt_use_numeric_sort_on_gv_col(this, nCol) || (nCol == 0 && IsNumericHeader()))
...
int CALLBACK CGridCtrl::pfnCellNumericCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	CGridCellBase* pCell1 = (CGridCellBase*) lParam1;
	CGridCellBase* pCell2 = (CGridCellBase*) lParam2;
    wchar_t *StopString; // Gensym Addition
    double nValue1;      // Gensym Addition
    double nValue2;      // Gensym Addition

	if (!pCell1 || !pCell2) return 0;

    nValue1 = _tcstod(pCell1->GetText(), &StopString); // Gensym Addition
    nValue2 = _tcstod(pCell2->GetText(), &StopString); // Gensym Addition
    ...
    while (lo <= hi)
    {
        ...
        if (lo <= hi)
        {
            ...
            if (pfnCompare((LPARAM)GetCell(lo, nCol), (LPARAM)GetCell(hi, nCol), data) != 0)
            {
                ...
                UINT nRowTag = m_arRowTags[lo];	   // Gensym Addition
                m_arRowTags[lo] = m_arRowTags[hi]; // Gensym Addition
                m_arRowTags[hi] = nRowTag;		   // Gensym Addition
				InvalidateFindRowCache();          // Gensym Addition
            }
...
int CGridCtrl::GetRowTag(int nRow)
{
  if(nRow<0 || nRow >= m_nRows)
    return GENSYM_INVALID_ROW_TAG;

  return m_arRowTags[nRow];
}

void CGridCtrl::SetRowTag(int nRow, int tag)
{
  if(nRow<0 || nRow >= m_nRows)
    return;

  m_arRowTags[nRow] = tag;
  InvalidateFindRowCache();
}

int CGridCtrl::FindRow(int tag)
{
  if(tag == GENSYM_INVALID_ROW_TAG)
    return -1;
  ...
  for(int i=0; i<m_nRows; i++)
    if(m_arRowTags.GetAt(i) == tag)
      return (m_cacheRow = i);

  return (m_cacheRow = -1);
}

void CGridCtrl::InvalidateFindRowCache()
{
  m_cacheTag = GENSYM_INVALID_ROW_TAG;
  m_cacheRow = -1;
}
