...
void CInPlaceEdit::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	// Moved from OnChar Gensym -fmw, 9/21/06 -----------------------
    if (nChar == VK_TAB || (nChar == VK_RETURN && 
							// Don't exit on Ctrl+Enter if we are multiline.
							!((GetKeyState(VK_CONTROL) < 0) && (GetStyle() & ES_MULTILINE))))
    {
        m_nLastChar = nChar;
...
{
// Moved to OnKeyDown Gensym -fmw, 9/21/06
//     if (nChar == VK_TAB || nChar == VK_RETURN)
//     {
//         m_nLastChar = nChar;
//         GetParent()->SetFocus();    // This will destroy this window
...
//     }
	...
	// This is more difficult for a multiline control, so skip it in that case.
	// The autoresizing is non-standard anyways; autohscroll takes care of it.
...
// EndEdit initiated by a private windows message
LRESULT CInPlaceEdit::OnEndEditMsg(WPARAM,LPARAM)
{
  EndEdit();
  return 1;
}
