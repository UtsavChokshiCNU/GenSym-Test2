...
BOOL CGridCell::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
	...
	{   
		...
		m_pEditWnd = CreateInPlaceEditor(pGrid, rect, dwStyle, nID, nRow, nCol, GetText(), nChar);
    }
    return TRUE;
}

CWnd *CGridCell::CreateInPlaceEditor(CWnd* pParent, CRect& rect, DWORD dwStyle, UINT nID,
									 int nRow, int nColumn, CString sInitText, 
									 UINT nFirstChar)
{
  return new CInPlaceEdit(pParent, rect, dwStyle, nID, nRow, nColumn, sInitText, nFirstChar);
}

void CGridCell::EndEdit()
{
  if (m_pEditWnd)
	m_pEditWnd->SendMessage(WM_END_EDIT);
}
