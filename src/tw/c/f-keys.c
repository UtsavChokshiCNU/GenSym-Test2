...
Object convert_key_symbol_to_key_code varargs_1(int, n)
{
    ...
    key_symbol = REQUIRED_ARG_nonrelocating();
    is_mouse_button_qm = OPTIONAL_ARG_P_nonrelocating() ? 
	    OPTIONAL_ARG_nonrelocating() : Nil;
    END_ARGS_nonrelocating();
    if (is_mouse_button_qm) {
	if (EQ(key_symbol,Qany))
	    key_symbol = Qmouse_down;
	else if (EQ(key_symbol,Qleft))
	    key_symbol = Qmouse_left_down;
	else if (EQ(key_symbol,Qmiddle))
	    key_symbol = Qmouse_middle_down;
	else if (EQ(key_symbol,Qright))
	    key_symbol = Qmouse_right_down;
    }
    if (EQ(SYMBOL_PACKAGE(key_symbol),Keyword_package_1))
	key_symbol = intern(SYMBOL_NAME(key_symbol),Ab_package_1);
    temp = getfq_function_no_default(INLINE_SYMBOL_PLIST(key_symbol),
...
Object Twrite_control_as_ctrl = UNBOUND;
    ...
    Object key_code;
{
	...
	PUSH_SPECIAL(Twrite_control_as_ctrl,twrite_control_as_ctrl,0);
