...
Object report_listener_info_to_parent_window()
{
	...
	ab_loop_list_ = M_CDR(ab_loop_list_);
	ab_loopvar__2 = gensym_cons_1(length(network_type),Nil);
	if (ab_loopvar__1)
	...
	reclaim_gensym_list_1(left_hand_slot_lengths_qm);
...
Object System_window_title_and_info_text_string_qm = UNBOUND;
    ...
    Object icp_socket;
{
    ...
    PUSH_SPECIAL(Current_system_name,current_system_name,0);
      if (icp_input_port) {
...
Object Gsi_throw_to_run_loop_may_be_harmful_p = UNBOUND;
    ...
    Object icp_socket;
{
    ...
    PUSH_SPECIAL(Gsi_throw_to_run_loop_may_be_harmful_p,gsi_throw_to_run_loop_may_be_harmful_p,
	    0);
...
Object Icp_socket_for_reclaim_qm = UNBOUND;
    ...
    Object icp_port, ack_p, actual_index_space_name;
{
    ...
    PUSH_SPECIAL(Current_icp_socket,current_icp_socket,0);
	  ...
	  if (ack_p)
		  ...
		  (SI_Long)22L)) >= (SI_Long)12L ?  !TRUEP(Nil) : TRUEP(T)))
	      temp = Nil;
				  ...
				  PUSH_SPECIAL(Current_write_icp_byte_position,current_write_icp_byte_position,
					...
					if (icp_connection_open_p(Current_icp_socket) 
						...
						send_icp_deregister_icp_objects(index_space_name,
							...
							index_list);
						reclaim_gensym_list_1(index_list);
					    }
					    else
    ...
    Object local_p, will_ack_p;
{
	...
	entry = gensym_cons_1(index_space_name,Nil);
	temp = gensym_cons_1(entry,alist);
	...
	entry = gensym_cons_1(type,Nil);
	temp = gensym_cons_1(entry,CDR(index_space_name_entry));
	...
	future_task_schedule_modify_arg_3 = 
		DFLOAT_ISAREF_1(Current_real_time,(SI_Long)0L) + temp_1;
	future_task_schedule_modify_arg_8 = 
		SYMBOL_FUNCTION(Qsend_queued_reclaim_index_space_messages);
    ...
    Object icp_socket;
{
    ...
    next_loop:
      ...
      reclaim_gensym_cons_1(index_space_element);
      element = Nil;
			  ...
			  PUSH_SPECIAL(Current_icp_buffer,current_icp_buffer,
				...
				PUSH_SPECIAL(Writing_icp_message_group,writing_icp_message_group,
				  ...
				  if (icp_connection_open_p(Current_icp_socket) 
					  ...
					  !TRUEP(icp_connection_being_shutdown_qm(Current_icp_socket))) 
					      {
				      temp = ISVREF(Current_icp_socket,
					      (SI_Long)21L);
	...
	end_loop_2:;
	  ...
	  if (EQ(index_space_name,Qstandard_icp_object_index_space))
		  ...
		  if (EQ(ISVREF(port,(SI_Long)3L),Qoutput))
		      ...
		      actual_index_space_name = IFIX(FIXNUM_MIN(temp_1,
			      temp)) >= (SI_Long)21L ? Qitem_index_space : 
			      Qframe_serial_number;
		  }
	      }
	      else {
      ...
      reclaim_gensym_list_1(type_index_list);
      goto next_loop_1;
    end_loop_1:;
      reclaim_gensym_list_1(type_alist);
      goto next_loop;
    end_loop:
      reclaim_gensym_list_1(icp_objects_alist);
...
static Object string_constant_7;   /* "[message]" */

static Object Qmessage;            /* message */

static Object string_constant_8;   /* "resumable icp synch error: ~S expected, ~S found" */
...
Object send_icp_shut_down_data_server()
{
	...
	PUSH_SPECIAL(Index_of_top_of_backtrace_stack,index_of_top_of_backtrace_stack,
	  ...
	  if (Icp_write_trace_cutoff_level_qm) {
		  ...
		  emit_icp_write_trace(T,Nil,Nil,Qshut_down_data_server,
			  string_constant_7,Qmessage,Nil,Nil,Nil,Nil);
		...
		if (tracep)
		    emit_icp_write_trace(Nil,byte_count_before,
			    ...
			    Qshut_down_data_server,string_constant_7,
	...
	if (resumable_icp_state_1 ? TRUEP(Resumable_icp_state) : TRUEP(Nil)) {
		...
		result = icp_error_internal(Qerror,string_constant_8,
...
Object handle_icp_shut_down_data_server()
{
	...
	if (Icp_read_trace_cutoff_level_qm) {
		...
		emit_icp_read_trace(T,Nil,Nil,Qshut_down_data_server,
			string_constant_7,Qmessage,Nil,Nil,Nil,T);
	...
	if (resumable_icp_state_1 ? TRUEP(Resumable_icp_state) : TRUEP(Nil)) {
		...
		return icp_error_internal(Qerror,string_constant_8,
	...
	PUSH_SPECIAL(Index_of_top_of_backtrace_stack,index_of_top_of_backtrace_stack,
	  ...
	  if (Icp_write_trace_cutoff_level_qm) {
		...
		if (tracep)
		    emit_icp_write_trace(Nil,byte_count_before,
			    instantaneous_icp_writer_byte_count(),
			    Qacknowledge_shutdown,string_constant_7,
			    Qmessage,Nil,Nil,Nil,Nil);
	      POP_SPECIAL();
	  }
	POP_SPECIAL();
...
Object handle_icp_acknowledge_shutdown__body()
{
    ...
    SVREF(svref_arg_1,FIX((SI_Long)9L)) = Qrequested;
    shutdown_icp_socket_connection(1,Current_icp_socket);
    return VALUES_1(Nil);
}
...
Object handle_icp_acknowledge_shutdown()
{
	...
	if (Icp_read_trace_cutoff_level_qm) {
		...
		result_of_read = handle_icp_acknowledge_shutdown__body();
		  ...
		  emit_icp_read_trace(Nil,byte_count_before,
			  instantaneous_icp_reader_byte_count(),
			  Qacknowledge_shutdown,string_constant_7,Qmessage,
...
Object In_recursive_gsi_context = UNBOUND;
    ...
    Object icp_socket;
{
	...
	if (icp_connection_open_p(icp_socket)) {
		...
		future_task_schedule_modify_arg_8 = 
			SYMBOL_FUNCTION(Qtime_out_icp_socket);
		future_task_schedule_modify_arg_11 = icp_socket;
...
Object Check_icp_handshaking_p = UNBOUND;
...
void int4_INIT()
{
    ...
    Object Qremove_icp_port_entries_from_icp_object, Qint4;
    Object Qsystem_window_title_and_info_text_string_qm;
    Object Qgeneric_listener_initialized_qm, list_constant_1;
    Object Qget_unix_host_name;

    x_note_fn_call(49,50);
    SET_PACKAGE("AB");
    ...
    string_constant_7 = STATIC_STRING("[message]");
