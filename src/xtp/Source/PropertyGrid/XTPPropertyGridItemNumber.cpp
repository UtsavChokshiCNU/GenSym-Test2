...
#include "StdAfx.h"
#include <math.h> // Gensym addition -fmw, 6/22/06
...
{
	 	// Gensym change: round instead of truncate entered values -fmw, 3/1/06
	// SetNumber(_ttol(strValue));
 	TCHAR *ep;
 	double d = _tcstod((LPCTSTR)strValue, &ep);
 	if(*ep == 0)
 		SetNumber((int) floor(d+0.5));
 	else
 		::MessageBeep(MB_ICONEXCLAMATION); // TODO: Only if interactive.
}
...
void CXTPPropertyGridItemDouble::SetFormat(LPCTSTR strFormat)	// Gensym -fmw, 3/5/06
{
	CXTPPropertyGridItem::SetFormat(strFormat);
	SetDouble(m_fValue);
}
