...
BOOL CXTPImageManagerIcon::GetBitmapBits(CDC& dcSrc, HBITMAP hBitmap, PBITMAPINFO& pBitmapInfo, LPVOID& pBits, UINT& nSize)
{
	...
	int nBytes = sizeof(BITMAPINFOHEADER) + 3 * sizeof(COLORREF); // Gensym
	if ((pBitmapInfo = (PBITMAPINFO)malloc(nBytes)) == NULL)       // Gensym
	{
		FREE(pBits);
		return FALSE;
	}

	ZeroMemory(pBitmapInfo, nBytes);                               // Gensym -fmw, 5/30/07
