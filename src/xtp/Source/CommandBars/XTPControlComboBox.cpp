...
CXTPControlPopup* g_69_ControlPopup;
...
CXTPControlComboBox::CXTPControlComboBox(CXTPCommandBars* pCommandBars)
{
	...
	m_nWidth = 100;
	m_nLabelWidth = 0;
	m_nThumbWidth = 15;						// Gensym -fmw, 5/30/07
	m_pEdit = NULL;
	m_strEditText = _T("");
...
void CXTPControlComboBox::OnExecute()
{
	...
	//{{suzsoft changed for Gensym-2094,
	if (kskr_popup != 2)
		m_nLastSel = GetCurSel();
	else
	{
		// if the kskr_popup is two, it means that the combo box is automatically closed.
		// we can't change the current selected in the control
		kskr_popup = 0;
		int nTemp = GetCurSel();
		if (nTemp != m_nLastSel)
			SetCurSel(m_nLastSel);
...
void CALLBACK CXTPControlComboBoxList::KeepaliveTimerProc(HWND hWnd, UINT /*uMsg*/, UINT idEvent, DWORD /*dwTime*/)
{
}
...
BOOL CXTPControlComboBoxList::SetTrackingMode(int bMode, BOOL bSelectFirst, BOOL bKeyboard)
{
	...
	{
		m_keepaliveId = ::SetTimer(NULL, 0, 100, (TIMERPROC)KeepaliveTimerProc);
...
void CXTPControlComboBoxList::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if ((kskr_popup -2  == 0) && (XTP_gensym_69 ==1))
	{
		pComboBox->OnExecute();
		//CXTPControlPopup::OnSetPopup(0);
		kskr_popup=0;
		return;
	}
...
BOOL CXTPControlComboBoxList::Popup(CXTPControlPopup* pControlPopup, BOOL /*bSelectFirst*/)
{
	...
	g_69_ControlPopup = pControlPopup;
	...
	if ((XTP_gensym_69 == 1) && (kskr_popup == 0))
	{
		kskr_popup=1;
	}
