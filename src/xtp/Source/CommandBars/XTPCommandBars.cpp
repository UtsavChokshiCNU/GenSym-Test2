...
UINT CXTPCommandBars::ReleaseToolbarContextMenu(LPVOID PopupBar)
{
    CXTPPopupBar* pPopup = (CXTPPopupBar*)PopupBar;
	
     while (pPopup->IsTrackingMode())
	 {
		Sleep(5);
	 }
	pPopup->GetSite()->SendMessage(WM_XTP_RELEASE_POPUP, 0, (LPARAM)pPopup);
	return 0;
}
//End of adding
...
void CXTPCommandBars::ContextMenu(CXTPToolBar* pToolBar, CPoint point)
{
	...
	if (pPopup && pPopup->GetControlCount() > 0)
	{
		...
		ClosePopups();
		//Changes for gensym-3031 by Suzsoft
		if (!pPopup->Popup(point.x, point.y, NULL))
		{ 
			return;
		}
		AfxBeginThread(ReleaseToolbarContextMenu, pPopup);
		//End of Changes
	}
