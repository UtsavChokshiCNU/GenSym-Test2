...
public:
	CControlMDIButton()
	{
		...
		m_dwFlags &= ~xtpFlagManualUpdate; // Gensym change: remove xtpFlagManualUpdate flag. -fmw, 2/2/06
	}
...
void CXTPMenuBar::RefreshSysButtons()
{
	...
	//DWORD dwStyle = hWndActiveChild ? GetWindowLong(hWndActiveChild, GWL_STYLE) : 0; // Gensym
	CXTPControl*  pButton;
	...
	if (bMax && (m_dwFlags & xtpFlagAddMDISysPopup))
	{
 	...
 	// Gensym change (three places below): add the button even if it would be
 	// disabled. -fmw, 2/2/06
...
int CXTPMenuBar::OnHookMessage(HWND hWnd, UINT nMessage, WPARAM& wParam, LPARAM& lParam, LRESULT& lResult)
{
	...
	switch (nMessage)
	{
		case WM_SYSCOMMAND:
			...
			{
				// Gensym addition. -fmw, 6/21/04
				if(GetSite()->SendMessage(WM_XTP_INITMENUBAR, 0, (LPARAM)this) == GENSYM_XTP_ABORT_MENU)
					break;
...
BEGIN_MESSAGE_MAP(CXTPMenuBar, CXTPToolBar)
	ON_WM_TIMER()
	ON_WM_LBUTTONDOWN()						// Gensym addition. -fmw, 6/21/04
	ON_WM_DESTROY()
END_MESSAGE_MAP()
...
// Gensym addition. -fmw, 6/21/04
void CXTPMenuBar::OnLButtonDown(UINT nFlags, CPoint point)
{
	if(GetSite()->SendMessage(WM_XTP_INITMENUBAR, 0, (LPARAM)this) != GENSYM_XTP_ABORT_MENU)
