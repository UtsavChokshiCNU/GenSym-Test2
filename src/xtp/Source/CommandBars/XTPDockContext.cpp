...
void CXTPDockContext::Track()
{
	...
	m_pBar->GetSite()->SendMessage(WM_XTP_MODALLOOP, 1, 0); // Gensym addition. -fmw, 7/29/04
	m_pBar->SetCapture();
	...
	m_pBar->GetSite()->SendMessage(WM_XTP_MODALLOOP, 0, 0); // Gensym addition. -fmw, 7/29/04
}
