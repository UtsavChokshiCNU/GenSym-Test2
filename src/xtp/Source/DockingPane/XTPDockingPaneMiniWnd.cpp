...
{
	m_bSysTracking = FALSE;				// Gensym -- MFC bugfix -fmw, 6/1/07
...
BEGIN_MESSAGE_MAP(CXTPDockingPaneMiniWnd, CMiniFrameWnd)
	ON_WM_CLOSE()
	ON_WM_MOVE()		// Gensym addition -fmw, 8/30/05
	ON_WM_NCLBUTTONDOWN()
	ON_WM_NCRBUTTONUP()
...
// Gensym addition -fmw, 8/30/05
void CXTPDockingPaneMiniWnd::OnMove(int /*x*/, int /*y*/)
{
	if (m_pTopContainer)
		{	
			CXTPDockingPaneBaseList lst;
			...
			while (pos) {
				CXTPDockingPane* pPane = (CXTPDockingPane*)lst.GetNext(pos);
				GetDockingPaneManager()->NotifyAction(xtpPaneActionMoved, pPane); // Not cancellable
			}
		}
}
