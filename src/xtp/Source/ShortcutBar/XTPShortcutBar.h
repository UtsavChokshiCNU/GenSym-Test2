...
{
	DECLARE_DYNCREATE(CXTPShortcutBar) // Gensym addition -fmw, 10/13/05
...
//-----------------------------------------------------------------------
// Summary:
//     CXTPShortcutBarThemeManager is the manager class of shortcut bar
//     themes
//-----------------------------------------------------------------------

class CXTPShortcutBarThemeManager
{
public:
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this method to get instance of theme manager
	//-----------------------------------------------------------------------
	static CXTPShortcutBarThemeManager* GetThemeManager();

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this method to set theme of shortcut bar
	// Parameters:
	//     xtpShortcutTheme - the theme to set
	//-----------------------------------------------------------------------
	void SetTheme(XTPShortcutBarPaintTheme xtpShortcutTheme);

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this method to get current theme
	//-----------------------------------------------------------------------
	XTPShortcutBarPaintTheme GetCurrentTheme();

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this method to create paint manager of the specific theme
	// Parameters:
	//     xtpShortcutTheme - the theme to create
	//-----------------------------------------------------------------------
	void CreatePaintManager(XTPShortcutBarPaintTheme xtpShortcutTheme);

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this method to get current paint manager
	//-----------------------------------------------------------------------
	CXTPShortcutBarPaintManager* GetPaintManager();

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this method to add a CXTPShortcutBar into managed list
	// Parameters:
	//     pBar - CXTPShortcutBar to be added into the managed list
	//-----------------------------------------------------------------------
	void AddShortcutBarIntoThemeManager(CXTPShortcutBar* pBar);

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this method to remove a CXTPShortcutBar
	// Parameters:
	//     pBar - CXTPShortcutBar to be removed from the managed list
	//-----------------------------------------------------------------------
	void RemoveShortcutBarFromThemeManager(CXTPShortcutBar* pBar);

private:
	//-----------------------------------------------------------------------
	// Summary:
	//     Construct a CXTPShortBarThemeManager object
	//-----------------------------------------------------------------------
	CXTPShortcutBarThemeManager();

	//-----------------------------------------------------------------------
	// Summary:
	//     Destruct a CXTPShortcutBarThemeManager object
	//-----------------------------------------------------------------------
	~CXTPShortcutBarThemeManager();

private:
	CXTPShortcutBarPaintManager* m_currentPaintManager;		// current paint manager for shortcut bar
	XTPShortcutBarPaintTheme m_currentTheme;				// current theme
	std::vector<CXTPShortcutBar*> *m_managedXTPShortcutBarList;	// list of current managed shortcut bar
	static CXTPShortcutBarThemeManager* m_instance;			// self instance of CXTPShortcutBarThemeManager
};
