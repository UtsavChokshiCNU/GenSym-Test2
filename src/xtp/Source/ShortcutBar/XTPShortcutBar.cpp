...
{
	m_paintTheme = CXTPShortcutBarThemeManager::GetThemeManager()->GetCurrentTheme();
	m_pPaintManager = CXTPShortcutBarThemeManager::GetThemeManager()->GetPaintManager();
	...
	CXTPShortcutBarThemeManager::GetThemeManager()->AddShortcutBarIntoThemeManager(this);
}

CXTPShortcutBar::~CXTPShortcutBar()
{
	// current paint manager was managed by CXTPShortcutBarThemeManager
	// CMDTARGET_RELEASE(m_pPaintManager);
	CXTPShortcutBarThemeManager::GetThemeManager()->RemoveShortcutBarFromThemeManager(this);
...
{
	// Theme for CXTPShortcutBar was managed by CXTPShortcutBarThemeManager
	/*
	CMDTARGET_RELEASE(m_pPaintManager);
...
void CXTPShortcutBar::SetCustomTheme(CXTPShortcutBarPaintManager* pPaintManager)
{
	...
	// currently, paint manager was managed by CXTPShortcutBarThemeManager
	// CMDTARGET_RELEASE(m_pPaintManager);
...
IMPLEMENT_DYNCREATE(CXTPShortcutBar, CWnd) // Gensym addition -fmw, 10/12/05
...
// Implementation for CXTPShortcutBarThemeManager

CXTPShortcutBarThemeManager* CXTPShortcutBarThemeManager::m_instance = NULL;

CXTPShortcutBarThemeManager::CXTPShortcutBarThemeManager()
{
	m_managedXTPShortcutBarList = new std::vector<CXTPShortcutBar*>();
	CXTPShortcutBarThemeManager::m_currentPaintManager = NULL;
	CreatePaintManager(xtpShortcutThemeOffice2003);
}

CXTPShortcutBarThemeManager* CXTPShortcutBarThemeManager::GetThemeManager()
{
	if (m_instance == NULL)
		m_instance = new CXTPShortcutBarThemeManager();
	return m_instance;
}

CXTPShortcutBarThemeManager::~CXTPShortcutBarThemeManager()
{
	if (m_managedXTPShortcutBarList != NULL) {
		m_managedXTPShortcutBarList->empty();
		delete m_managedXTPShortcutBarList;
		m_managedXTPShortcutBarList = NULL;
	}

	if (m_currentPaintManager != NULL) {
		delete m_currentPaintManager;
		m_currentPaintManager = NULL;
	}

	m_instance = NULL;
}

void CXTPShortcutBarThemeManager::SetTheme(XTPShortcutBarPaintTheme xtpShortcutTheme)
{
	CXTPShortcutBarPaintManager *originalPaintManager = m_currentPaintManager;

	CreatePaintManager(xtpShortcutTheme);

	std::vector<CXTPShortcutBar*>::iterator firstBar = m_managedXTPShortcutBarList->begin();
	std::vector<CXTPShortcutBar*>::iterator lastBar = m_managedXTPShortcutBarList->end();
	std::vector<CXTPShortcutBar*>::iterator iter;
	for (iter = firstBar; iter != lastBar; iter++) {
		CXTPShortcutBar* pBar = *iter;
		pBar->SetCustomTheme(m_currentPaintManager);
	}

	if (originalPaintManager != NULL)
		delete originalPaintManager;
}

XTPShortcutBarPaintTheme CXTPShortcutBarThemeManager::GetCurrentTheme()
{
	return m_currentTheme;
}

void CXTPShortcutBarThemeManager::CreatePaintManager(XTPShortcutBarPaintTheme xtpShortcutTheme)
{
	switch (xtpShortcutTheme) {
		case xtpShortcutThemeOfficeXP:
			m_currentPaintManager = new CXTPShortcutBarOfficeXPTheme();
			break;
		case xtpShortcutThemeOffice2003:
			m_currentPaintManager = new CXTPShortcutBarOffice2003Theme();
			break;
		case xtpShortcutThemeOffice2007:
			m_currentPaintManager = new CXTPShortcutBarOffice2007Theme();
			break;
		case xtpShortcutThemeNativeWinXP:
			m_currentPaintManager = new CXTPShortcutBarNativeWinXPTheme();
			break;
		default:
			m_currentPaintManager = new CXTPShortcutBarOffice2000Theme();
			break;
	}
	m_currentTheme = xtpShortcutTheme;
}

CXTPShortcutBarPaintManager* CXTPShortcutBarThemeManager::GetPaintManager()
{
	return m_currentPaintManager;
}

void CXTPShortcutBarThemeManager::AddShortcutBarIntoThemeManager(CXTPShortcutBar* pBar)
{
	m_managedXTPShortcutBarList->push_back(pBar);
}

void CXTPShortcutBarThemeManager::RemoveShortcutBarFromThemeManager(CXTPShortcutBar* pBar)
{
	std::vector<CXTPShortcutBar*>::iterator firstBar = m_managedXTPShortcutBarList->begin();
	std::vector<CXTPShortcutBar*>::iterator lastBar = m_managedXTPShortcutBarList->end();
	std::vector<CXTPShortcutBar*>::iterator iter;
	for (iter = firstBar; iter != lastBar; iter++)
		if (*iter == pBar)
			break;
	if (iter != lastBar)
		m_managedXTPShortcutBarList->erase(iter);
}