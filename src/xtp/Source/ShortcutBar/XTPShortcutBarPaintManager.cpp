...
int CXTPShortcutBarOfficeXPTheme::DrawPaneCaption(CDC* pDC, CXTPShortcutBarPane* pPane, BOOL bDraw)
{
	if (bDraw)
	{
		CXTPFontDC font(pDC, &m_fntPaneCaption);
...
int CXTPShortcutBarNativeWinXPTheme::DrawPaneCaption(CDC* pDC, CXTPShortcutBarPane* pPane, BOOL bDraw)
{
	if (bDraw)
	{
		if (!m_themeRebar->IsAppThemed())
		{
			return CXTPShortcutBarOffice2003Theme::DrawPaneCaption(pDC, pPane, bDraw);
		}
