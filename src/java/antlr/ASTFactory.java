...
public AST create() {
	...
	AST srcTree = t;
	AST result = dupTree(srcTree);            // if srcTree == null, then result==null
	AST nt = result;
	while (srcTree != null) {						// for each sibling of the root
		srcTree = srcTree.getNextSibling();
		nt.setNextSibling(dupTree(srcTree));	// dup each subtree, building new tree
