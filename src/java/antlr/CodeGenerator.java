...
public abstract class CodeGenerator {
	...
	public static final String TokenTypesFileSuffix = "TokenTypes";
	public static final String TokenTypesFileExt = ".txt";
	 ...
	 */
	protected String processActionForTreeSpecifiers(final String actionStr, final int line, final RuleBlock currentRule, final ActionTransInfo tInfo) {
		if ( actionStr==null ) {
			return null;
		}
		...
		boolean grammerCheck = grammar instanceof TreeWalkerGrammar ||
			     (grammar instanceof LexerGrammar && action.indexOf('$') != -1);
		
		if ( (grammar.buildAST && action.indexOf('#') != -1) || grammerCheck ) {
			// Create a lexer to read an action and return the translated version
			antlr.actions.ActionLexer lexer = new antlr.actions.ActionLexer(action, currentRule, this, tInfo);
			lexer.setLineOffset(line);
			lexer.setTool(tool);
			try {
				lexer.mACTION(true);
				action = lexer.getTokenObject().getText();
				...
				antlr.Tool.panic("IO error reading action:"+action);
				return action;
			}
		}
		return action;
