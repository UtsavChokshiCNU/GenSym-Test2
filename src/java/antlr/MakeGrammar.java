...
*/
	...
	private AlternativeBlock createOptionalRuleRef(final String rule, final int line) {
		// Make the subrule
		AlternativeBlock blk = new AlternativeBlock(grammar, line, false);
		String ruleStr = rule;
		// Make sure rule is defined
		if ( Character.isUpperCase(ruleStr.charAt(0)) ) { // lexer rule?
			ruleStr = CodeGenerator.lexerRuleName(ruleStr);
		}
		if ( !grammar.isDefined(ruleStr) ) {
			grammar.define(new RuleSymbol(ruleStr));
		}
		
		// Make the rule ref element
		RuleRefElement rref = new RuleRefElement(grammar, ruleStr, line, GrammarElement.AUTO_GEN_NONE);
