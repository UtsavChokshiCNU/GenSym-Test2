...
public abstract class BaseAST implements AST {
	...
	private static volatile boolean verboseStringConversion = false;
	private static volatile String[] tokenNames = null;
...
*/
	...
	public boolean equals(Object t) {
		if ( t==null || !getClass().equals(t.getClass()) ) 
			return false;
 ...
 */
public boolean equalsList(final AST t) {
	AST sibling;
	AST ast = t;
	...
	for (sibling = this; sibling != null && ast != null; sibling = sibling.getNextSibling(), ast = ast.getNextSibling()) {
		// as a quick optimization, check roots first.
		if (!sibling.equals(ast)) {
			return false;
		}
		// if roots match, do full list match test on children.
		if (sibling.getFirstChild() != null) {
			if (!sibling.getFirstChild().equalsList(ast.getFirstChild())) {
				return false;
			}
		}
 ...
 */
public boolean equalsListPartial(final AST sub) {
	AST sibling;
	AST subtree = sub;
	...
	// Otherwise, start walking sibling lists.  First mismatch, return false.
	for (sibling=this;
		  sibling!=null&&subtree!=null;
		  sibling=sibling.getNextSibling(), subtree=subtree.getNextSibling())
	{
		// as a quick optimization, check roots first.
		if ( !sibling.equals(subtree) ) 
			return false;
		// if roots match, do partial list match test on children.
		if ( sibling.getFirstChild()!=null ) {
			if ( !sibling.getFirstChild().equalsListPartial(subtree.getFirstChild()) ) 
				return false;
		}	
	}
	if ( sibling==null && subtree!=null ) {
