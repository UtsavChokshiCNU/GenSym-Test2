...
private void removeCompetingPredictionSets(BitSet b, AlternativeElement el) {
	...
	if(el == null){
		System.out.println("Element (el) is NULL.");
	}
	if ( head instanceof TreeElement ) {
		if ( !el.equals(((TreeElement) head).root) ) {
			return;
		}
	}
	else if ( !el.equals(head) ) {
		return;
	}
	...
	public boolean subruleCanBeInverted(AlternativeBlock blk, boolean forLexer)
	{
		...
		for (int i = 0; i < blk.alternatives.size(); i++) {
			...
			boolean elementTypeCheck = elt instanceof CharLiteralElement || elt instanceof TokenRefElement 
					|| elt instanceof CharRangeElement || elt instanceof TokenRangeElement;
			elementTypeCheck = elementTypeCheck || (elt instanceof StringLiteralElement && !forLexer);
