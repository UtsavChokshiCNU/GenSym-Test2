...
public class CppCodeGenerator extends CodeGenerator {
	...
	public void gen(RuleRefElement rr) {
		...
		if (syntacticPredLevel == 0) {
			...
			boolean checkAST = grammar.buildAST && rr.getLabel() != null || 
					(genAST && rr.getAutoGenType() == GrammarElement.AUTO_GEN_NONE);
			
			boolean doNoGuessTest = grammar.hasSyntacticPredicate && checkAST;
...
//		}
	...
	protected void genSemPred(final String pred, final int line) {
		// translate $ and # references
		ActionTransInfo tInfo = new ActionTransInfo();
		String predicate = processActionForTreeSpecifiers(pred, line, currentRule, tInfo);
		// ignore translation info...we don't need to do anything with it.
		String escapedPred = charFormatter.escapeString(predicate);
		...
		if (grammar.debuggingOutput && ((grammar instanceof ParserGrammar) ||
			...
			predicate = "fireSemanticPredicateEvaluated(antlr.debug.SemanticPredicateEvent.VALIDATING,"
				+ addSemPred(escapedPred) + "," + predicate + ")";
		println("if (!(" + predicate + "))");
		tabs++;
...
//		println("");
	...
	public String mapTreeId(String idParam, ActionTransInfo transInfo) {
		...
		if (s != null) {
			if (NONUNIQUE.equals(s) || s.equals(currentRule.getRuleName())) {
				// There is more than one element with this id
				// OR a recursive call to the enclosing rule is 
