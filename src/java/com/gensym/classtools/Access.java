...
{ 	
 ...
 public boolean isVolatile() {
  ...
  public String toString() {
    ...
    if (isPublic()){
    	stringHelper(buf,"public");
    }
    else if(isPrivate()){
    	stringHelper(buf,"private");
    }
    else if(isProtected()){
    	stringHelper(buf,"protected");
    }
    
    if (isStatic()){
    	stringHelper(buf,"static");
    }
    if (isFinal()){
    	stringHelper(buf,"final");
    }
    if(isSynchronized()) {
    ...
    if (isVolatile()){
    	stringHelper(buf,"volatile");
    }
    if (isTransient()){
    	stringHelper(buf,"transient");
    }
    if (isNative()) {
    	stringHelper(buf,"native");
    }
    if (isAbstract()) {
    	stringHelper(buf,"abstract");
    }
    if (isInterface()){
    	stringHelper(buf,"interface");
    }
