...
public class SimpleWorkspaceApplication extends UiApplication {
  ...
  private MultipleWorkspaceView multiWkspView;
...
class WorkspaceDownloadThread extends Thread {
   ...
   * Save Data
   */
  WorkspaceDownloadThread (MultipleWorkspaceView multiWkspView,
			   TwGateway cxn,
			   Symbol wkspName_) {
    this.multiWkspView = multiWkspView;
    this.cxn = cxn;
  ...
  public void run () {
    try {
      ...
      multiWkspView.addWorkspace (wksp, true);
      multiWkspView.setCurrentWorkspace (wksp);
    } catch (G2AccessException gae) {
      new WarningDialog (null, "Error during Get Workspace", true, gae.toString (), null).setVisible (true);
    }

    //Deregister default commands
    WorkspaceView wkspView = multiWkspView.getCurrentView();
    SelectionCommandGenerator generator = wkspView.getSelectionCommandGenerator();
    Vector registeredCommands = generator.getRegisteredCommands();
    for (int i=0; i<registeredCommands.size(); i++) {
      SelectionCommand sc = (SelectionCommand)registeredCommands.elementAt (i);
