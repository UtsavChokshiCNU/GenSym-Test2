...
rem is found in, <install directory>\classes\com\gensym\demos\itemaccessdemo
rem -or- 
...
rem of the planet FOOBAR, found on the solar-system workspace

@echo on
call "%SEQUOIA_HOME%\bin\start_g2" -kb "%SEQUOIA_HOME%\kbs\sq-demos.kb" -name "Sequoia Demo" -tcpport 1234 -tcpipexact

"%JAVA_HOME%\bin\java" com.gensym.demos.itemaccessdemo.ItemAccessDemo localhost 1234 FOOBAR
@echo off
goto end
:problem
