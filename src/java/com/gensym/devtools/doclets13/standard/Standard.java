...
import com.gensym.devtools.BuildUtils;
import com.gensym.devtools.ModuleInfo;
...
public class Standard {
  ...
  // gensym: New static methods and fields

  public static boolean excludeDoc(Doc doc) {
    if(ConfigurationStandard.noundocumented
       && doc.tags("undocumented").length != 0)
      return true;
    return false;
  }

  public static boolean excludeDoc(ClassDoc classDoc) {
    // NOTE. Ideally, the moduleInfo clauses will be unnecessary.
    // someone who uses this tool will have specified all the
    // correct packages, and the exclude-from-javadoc entry
    // in the package.info will have been retired in favor of simply
    // marking the class @undocumented (hence taken care of by the
    // first clause).
    if(excludeDoc((Doc)classDoc))
      return true;

    if(ConfigurationStandard.restrictToClassList
       && !ConfigurationStandard.classList.contains(classDoc.qualifiedName()))
	return true;

    if(ConfigurationStandard.rootPath != null) {
      ModuleInfo moduleInfo =
	BuildUtils.getModuleInfoForPackage(ConfigurationStandard.rootPath,
					   classDoc.containingPackage().name());
      if (moduleInfo != null) {
	boolean exclude = moduleInfo.excludeFromJavaDoc(classDoc.name() + ".java");
	return exclude;
      } else
	return false;
    } else
  ...
  protected void generateClassCycle(ClassDoc[] arr, ClassTree classtree,
    ...
    for(int i = 0; i < arr.length; i++) {
      if (excludeDoc(arr[i]) || //add check for undocumented
