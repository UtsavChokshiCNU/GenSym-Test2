...
public class ClassTree { 
  ...
  public static boolean noUndocumented = true;

  public boolean excludeClass(ClassDoc classDoc) {
    return isUndocumented(classDoc);
  }

  public boolean isUndocumented(ClassDoc classDoc) {
    return false;
  }
    ...
    private void buildTree(ClassDoc[] classes, boolean noDeprecated) {
        for (int i = 0; i < classes.length; i++) {
            if (com.gensym.devtools.doclets.standard.Standard.excludeClass(classes[i]) ||
		(noDeprecated && classes[i].tags("deprecated").length > 0) ||
		 classes[i].tags("undocumented").length > 0) {
                continue;
            }
