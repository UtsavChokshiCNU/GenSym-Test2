...
/**
 *  Lightweight version of StandardDialog
 */

public abstract class JStandardDialog extends javax.swing.JDialog implements CommandConstants {

  private JStandardDialogClient dlgClient;
  private CommandPanel cmdPanel;
  private Component dlgComponent;
  private int reasonForCallback;
  private static Frame theFrame;
  private static boolean checkedNtw;
   ...
   * of a JStandardDialog. The component passed in is placed in the
   ...
   * @param client a JStandardDialogClient that will be notified of all
   ...
   */
  protected JStandardDialog (Frame parent,
			    String title,
			    boolean isModal,
			    String[] btnLabels,
			    int[] cmdCodes,
    ...
    if (btnLabels != null) {
      cmdPanel = new CommandPanel (btnLabels, cmdCodes, cmdBtnListener, getButtonAlignment ());
   ...
   */
  private void setClient (JStandardDialogClient client) {
    dlgClient = client;
  }
  ...
  */
   ...
   * <code>Component</code> within the <code>JStandardDialog</code>
  ...
  protected void dispatchCallback (int code) {
    ...
    if (code == OK || code == CANCEL || code == YES || code == NO || code == DISMISS)
      dispose ();
  }

  /**
   * Indicates whether this dialog was cancelled by the user.
   * In the notification callback, JStandardDialogClients can
  ...
  private static Frame getFrame () {
    ...
    try {Thread.sleep (100);} catch (InterruptedException ie) {} // Stupid bug! Frame remains visible otherwise
...
class CommandPanel extends Panel {
  ...
  private Button desiredDefault, defaultBtn, cancelBtn;
  ...
  CommandPanel (String[] cmdNames, int[] cmdCodes, ActionListener listnr, int alignment) {
    setLayout (new FlowLayout (alignment));
    int numButtons = cmdNames.length;
    btns = new Button [numButtons];
    btnCodes = cmdCodes;
  ...
  public void paint (Graphics g) {
    ...
    if (defaultBtn != null) {
      Rectangle defaultBtnBounds = defaultBtn.getBounds ();
      g.setColor (Color.black);
      g.drawRect (defaultBtnBounds.x - 1, defaultBtnBounds.y - 1,
		  defaultBtnBounds.width + 1, defaultBtnBounds.height + 1);
    }
  }
  ...
  void refocusDefault (Button tempDefaultButton) {
    if (tempDefaultButton == defaultBtn)
      return;
    defaultBtn = (tempDefaultButton == null ? desiredDefault : tempDefaultButton);
    repaint ();
  }
}
...
class CommandButton extends Button {
  ...
  CommandButton (CommandPanel panel, String label, int code) {
    super (label);
    this.code = code;
    final CommandPanel cmdPanel = panel;
    addFocusListener (new FocusListener () {
      @Override
      public void focusGained (FocusEvent fEvt) {
	cmdPanel.refocusDefault (CommandButton.this);
      }
      @Override
      public void focusLost (FocusEvent fEvt) {
	cmdPanel.refocusDefault (null);
      }
    });
  }
...
class StandardDialogFrame extends Frame {
}
