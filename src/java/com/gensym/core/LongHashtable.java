 ...
 * optimization of using an long as the key, which avoids the
 * allocation of the wrapped Long in the corresponding case
 ...
 * correctly:
 * <pre> longHashtable.put (key.hashCode(), value); </pre>
 * @author  vkp
 * @version 1.00
 */
public class LongHashtable extends Dictionary implements Cloneable, java.io.Serializable {
  /**
   * The hash table data.
     */
  ...
  public synchronized boolean containsKey(long key) {
    LongHashtableEntry tab[] = table;
    long hash = key;
    int index = (((((int)hash) & 0x7FFFFFFF) ^ (((int)(hash >> 32)) & 0x7FFFFFFF))
		 % tab.length);
  ...
  protected void rehash() {
	...
	int index = (((((int)e.hash) & 0x7FFFFFFF) ^ (((int)(e.hash >> 32)) & 0x7FFFFFFF))
		     % newCapacity);
  ...
  public Object put(Object key, Object value) {
    return put (((Long)key).longValue(), value);
  }
       ...
       throws IOException
  {
    ...
    for (int index = table.length-1; index >= 0; index--) {
      LongHashtableEntry entry = table[index];

      while (entry != null) {
	s.writeLong(entry.hash);
       ...
       throws IOException, ClassNotFoundException
  {
    ...
    table = new LongHashtableEntry[length];
    count = 0;
 ...
 */
class LongHashtableEnumerator implements LongHashtableEnumeration {
  boolean keys;
  int index;
  ...
  public Object nextElement() {
    ...
    if (entry != null) {
      ...
      entry = e.next;
      return keys ? (Object) new Long (e.hash) : e.value;
    }
