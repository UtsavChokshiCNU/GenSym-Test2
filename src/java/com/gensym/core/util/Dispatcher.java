...
{
  public static class UnspecificReferenceClass 
  {
  ...
  public static boolean isReallyAssignableFrom(Class s, Class t) {
    if ((!s.isPrimitive() && t.equals(UnspecificReferenceClass.class)) 
    		|| s.isAssignableFrom(t)) 
    	return true;
    
    // P. 54, bottom for definition of widening primitive conversion
    boolean isNumber = t == plong || t == pint || t == pshort;
    if (s == pdouble) {
      return t == pfloat || isNumber || t == pchar || t == pbyte;
    }
      
    if (s == pfloat){
      return isNumber || t == pchar || t == pbyte;
    }
      
    if (s == plong){
      return t == pint || t == pshort || t == pchar || t == pbyte;
    }
     
    if (s == pint){
      return t == pshort || t == pchar || t == pbyte;
    }
    if (s == pshort){
      return t == pbyte;
    }
