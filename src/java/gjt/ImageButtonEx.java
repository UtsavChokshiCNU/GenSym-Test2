...
public class ImageButtonEx extends ImageButton {
  ...
  private Image           image, insetImage, disabledImage;
  ...
  public ImageButtonEx (Image image, Image insetImage) {
    this(image, insetImage, _defaultThickness, null);
  }
  public ImageButtonEx (Image image, 
			Image insetImage,
			ImageButtonController controller) {
    this(image, insetImage, _defaultThickness, controller);
  }
  public ImageButtonEx (Image image, Image insetImage, int thickness, 
			ImageButtonController controller) {
    super ();
    ...
    setImages(image, insetImage);
  }
  synchronized boolean imageIsLoaded (Image img) {
    if (true) return false;
    MediaTracker tracker = new MediaTracker(this);
    System.out.print ("In imageIsLoaded >>>>"); System.out.flush ();
    tracker.addImage(img, 0);
    boolean isLoaded = tracker.checkID (0,true);
    System.out.print ("imageIsLoaded ? = " + isLoaded); System.out.flush ();
    while (!tracker.checkID (0))
      try {
      wait (100);
    } catch (InterruptedException ie) {}
    System.out.println ("Exiting imageIsLoaded = " + tracker.checkID (0));
    return true;
  }
  public void setImages (Image image, Image insetImage) {
    if (!imageIsLoaded (image))
      Util.waitForImage(this, this.image = image);
    if (!imageIsLoaded (insetImage))
      Util.waitForImage(this, this.insetImage = insetImage);
  ...
  public void paintInset() {
    ...
    if(g != null) {
      ...
      g.drawImage(insetImage, 
		  upperLeft.x + thickness, 
