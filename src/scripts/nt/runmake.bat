...
rem  The values of the type-of-compile argument are either opt, o, o-pg, o-mt,
rem   or opt-intc.
rem   These correspond to directory holding binaries, and stand for optimized,
rem   debuggable, profiled, multithreaded, and intel compiler.
rem ***************************************************************************
...
if "%COMPILE_TYPE%" == "o-mt" goto got_type
if "%COMPILE_TYPE%" == "o-pg" goto got_type
if "%COMPILE_TYPE%" == "opt-intc" goto got_type
...
rem *** If not, warn and try one time to make the directory. ***
rem *** Then check once more, and quit if there's still no directory. ***
rem
:check_dir
