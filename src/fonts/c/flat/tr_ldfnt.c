 ...
 * $Header: /media/gensym/home/versata1/cvs_repo_copyDec06/ab-repos/fonts/c/flat/tr_ldfnt.c,v 1.1 1995-10-03 01:02:45 jra Exp $
 *
 * $Log: not supported by cvs2svn $
 * Revision 1.3  1995/03/14 00:31:35  mhd
 ...
 * development.
 *
 * Revision 1.2  1993/09/17  18:38:43  mhd
 * Changes to eliminate all compiler warnings in the fontrip/4-in-1 c files.
 *
 * Revision 1.1  1993/05/04  20:15:25  mhd
 * The flat-directory version of the Bitstream 4-in-1 software.
 *
 * Revision 1.3  1993/04/28  13:47:12  mhd
 * Fixed line in PROTOS_AVAIL-only code where a comma was missing (before the
 * identifier fix15).
 *
 * Revision 1.2  1993/04/23  22:32:35  mhd
 * I moved this from the inner block of a function to top level; static
 * function declarations are illegal in ANSI C:
 * 	static boolean  tr_read_long();
 *
 * Revision 1.1.1.1  1993/04/15  20:14:31  mhd
...
static char     rcsid[] = "$Header: /media/gensym/home/versata1/cvs_repo_copyDec06/ab-repos/fonts/c/flat/tr_ldfnt.c,v 1.1 1995-10-03 01:02:45 jra Exp $";
...
/* I moved this here from the inner block of the function below; static 
   function declarations are illegal in ANSI C.
   --mhd@gensym, 4/22/93
   */
static boolean  tr_read_long();  
...
#endif
{
	...
	/* a static function declaration in an inner block is illegal in ansi c,
	   at least as far the SGI (strict ansi) compiler is concerned.  I've moved
	   this to top-level, above this function.  -mhd@gensym, 4/22/93
	   */
	/* static boolean  tr_read_long(); */
	boolean         read_real();
	...
	void            clear_hints();
	/*
	   This declaration is confusing the gcc compiler
	   into giving these warnings:
             ./tr_ldfnt.c: In function `add_encoding':
	     ./tr_ldfnt.c:3119: warning: type mismatch with previous external decl
	     ./tr_ldfnt.c:813: warning: previous external decl of `malloc'
	     ./tr_ldfnt.c: In function `tr_set_encode':
	     ./tr_ldfnt.c:3498: warning: type mismatch with previous external decl
	     ./tr_ldfnt.c:813: warning: previous external decl of `malloc'
	   The declaration is not really needed, so I'm getting
	   rid of it to improve the warnings picture, although it
	   should not be necessary! -- mhd@gensym, 9/17/93
	   */
	/* void           *malloc();
	   */
	...
	/*
	   This declaration is confusing the gcc compiler
	   into giving these warnings:
	     ./tr_ldfnt.c: In function `read_byte':
	     ./tr_ldfnt.c:2723: warning: type mismatch with previous external decl
	     ./tr_ldfnt.c:805: warning: previous external decl of `get_byte'
	     ./tr_ldfnt.c:2742: warning: type mismatch with previous external decl
	     ./tr_ldfnt.c:805: warning: previous external decl of `get_byte'
	   The declaration is not really needed, so I'm getting
	   rid of it to improve the warnings picture, although it
	   should not be necessary! -- mhd@gensym, 9/17/93
		...
		if (compstr((ufix8 STACKFAR*)buffer, "/") == 0) {	/* / token? */
			...
			case 'F':
				if (compstr((ufix8 STACKFAR*)buffer, "FullName") == 0) {	/* /FullName? */
					...
					if ((compstr((ufix8 STACKFAR*)buffer, "{") != 0) && (strcmp(buffer, "[") != 0)) {
						tr_error(PARAMS2 TR_NO_READ_OPENBRACKET, local_font_ptr);	/* ERROR MESSAGE "***
													 * Cannot read { or [ in
													 * FontBBox" */
						return FALSE;
...
# endif
...
#endif
						STRcpy(local_font_ptr->charstrings[local_font_ptr->no_charstrings].key, charactername);
		...
		} else if (strcmp(buffer, "closefile") == 0) {	/* closefile? */
...
#else
	if ((font_ptr->encoding[index] = (CHARACTERNAME *) malloc(strlen(charactername) + 1)) == NULL)
		return FALSE;
#if WINDOWS_4IN1
	lstrcpy(font_ptr->encoding[index], charactername);
#else
	strcpy(font_ptr->encoding[index], charactername);
#endif
#endif
...
#else
	for (i = 0; i < sp_globals.processor.type1.current_font->no_charstrings; i++) {
		if (STRCMP(charname, sp_globals.processor.type1.current_font->charstrings[i].key) == 0) {
...
#else
#define CHSIZE strlen(set_array[i])+1
#endif
	 ...
	 */
	for (i = 0; i < 256; i++) {
		if (STRCMP(set_array[i], notdef) == 0) {
...
#endif
			STRCPY(sp_globals.processor.type1.current_font->encoding[i], set_array[i]);
		}
	}
	return TRUE;
#endif
