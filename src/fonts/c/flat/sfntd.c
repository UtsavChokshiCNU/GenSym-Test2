...
*                                                                                    *
*     $Header: /media/gensym/home/versata1/cvs_repo_copyDec06/ab-repos/fonts/c/flat/sfntd.c,v 1.1 1995-10-03 01:02:33 jra Exp $                                                                       *
*                                                                                    *
*     $Log: not supported by cvs2svn $
*     Revision 1.3  1995/03/14 00:31:20  mhd
...
*     development.
*
 * Revision 1.2  1993/09/17  18:38:41  mhd
 * Changes to eliminate all compiler warnings in the fontrip/4-in-1 c files.
 *
 * Revision 1.1  1993/05/04  20:15:13  mhd
...
#ifdef RCSSTATUS
static char rcsid[] = "$Header: /media/gensym/home/versata1/cvs_repo_copyDec06/ab-repos/fonts/c/flat/sfntd.c,v 1.1 1995-10-03 01:02:33 jra Exp $";
#endif
...
/* PRIVATE PROTOTYES */
/* added all of the static's that appear at the beginning of the lines below,
   to be consistent with function defs below. -mhd@gensym, 9/15/93
   */
static int sfnt_UnfoldCurve PROTO((fsg_SplineKey *key, sfnt_PackedSplineFormat *charData, int32 *sizeOfInstructions, uint8 **instructionPtr, int32 length, int16 *elementCount));
static void sfnt_Classify PROTO((fsg_SplineKey *key, sfnt_TableTag tag, int16 index));
static void* sfnt_GetDataPtr PROTO((fsg_SplineKey *key, int32 offset, int32 length, sfnt_tableIndex n, boolean mustHaveTable));
static uint16 sfnt_ComputeUnkownIndex PROTO((fsg_SplineKey *key, uint16 charCode));
#ifndef PCLETTO
static uint16 sfnt_ComputeIndex0 PROTO((fsg_SplineKey *key, uint16 charCode));
static uint16 sfnt_ComputeIndex2 PROTO((fsg_SplineKey *key, uint16 charCode));
static uint16 sfnt_ComputeIndex4 PROTO((fsg_SplineKey *key, uint16 charCode));
static uint16 sfnt_ComputeIndex6 PROTO((fsg_SplineKey *key, uint16 charCode));
static void sfnt_GetGlyphLocation PROTO(( fsg_SplineKey* key, uint16 gIndex, int32* offset, int32* length ));
#endif /* PCLETTO */
  ...
  int16* elementCount;
{
	...
	if ( *elementCount == GLYPHELEMENT ) {
		...
		fsg_IncrementElement( key, (int32)GLYPHELEMENT, key->numberOfRealPointsInComponent, (int32)elementPtr->nc );
	}
 ...
 *
 */
static uint16 sfnt_ComputeIndex2 (key, charCode)
 ...
 */
/* added static declaration here, since it's only used in this file. -mhd@gensym, 9/15/93
   */
static uint16 sfnt_ComputeIndex4 (key, charCode)
  fsg_SplineKey *key;
  uint16 charCode;
{
	void* mappingPtr = sfnt_GetTablePtr( key, sfnt_charToIndexMap, true );
...
/* added static declaration here, since it's only used in this file. -mhd@gensym, 9/15/93
   */
static uint16 sfnt_ComputeIndex6 (key, charCode)
  fsg_SplineKey* key;
  uint16 charCode;
{
	void* mappingPtr = sfnt_GetTablePtr( key, sfnt_charToIndexMap, true );
