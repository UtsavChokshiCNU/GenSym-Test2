...
*  Revision 1.2  1993/07/27 22:58:02  mhd
...
*  in detail.
*
 * Revision 1.1  1993/05/04  20:14:13  mhd
...
/*
  open_font_file_and_initialize opens the file designated by pathname
...
void open_font_file_and_initialize()
{
  char *first_string;
  int first_byte;
     ...
     char *argv[];
{
  ...
  open_font_file_and_initialize(pathname);
  ...
  for (char_index = 0; char_index < no_font_chars; char_index++)
    {
      ...
      if (character != (void *)0)
	{
	  printf("\nchar_index = %d; character = %d [%d]", char_index, character, (ufix16)character);
	  if (!fi_make_char(character))
	    {
	      printf("\nMISSING CHARACTER: %d\n", char_index);
	    }
	}
 ...
 ufix16 char_index;
{
  if (font_type == procType1)
    ...
    {
      printf("\nfont_table[char_index] has: %d", font_table[char_index]);
...
{  
  switch(n)
    ...
    case 12:
      fprintf(stderr, "\nUnknown error case (12) [Report character data not avail char index: %c (%d); character: %d]",
